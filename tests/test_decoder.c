...
main(void)
{
    ...
    for (uint32_t hash_index = 0; hash_index < DECODER_HASH_TABLE_SIZE; ++hash_index)
    {
        ...
        if (bucket->count != 0)
        {
            ...
            {
                printf("[FATAL] Bucket %u has %hhu items. Increase MAX_LOCAL_CANDIDATES.\n",
                       hash_index,
                       bucket->count);
                return 1;
            }
        ...
        for (uint32_t offset = 0; offset < BLOCK_SIZE; ++offset)
        {
            ...
            // Device Under Test.
            //
            const bal_decoder_instruction_metadata_t *dut_result = bal_decode_arm64(instruction);
                ...
                if ((instruction & local_candidates[k].mask) == local_candidates[k].expected)
                {
                    ...
                    if (BAL_UNLIKELY(k + 1 < candidate_count))
                    {
                        if (local_candidates[k].priority == local_candidates[k + 1].priority)
                        {
                            ...
                            {
                                if (strcmp(ref_result->name, local_candidates[k + 1].metadata->name)
                                    != 0)
                                {
                                    ++total_collisions;
                                }
