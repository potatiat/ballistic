...
#include "unity_internals.h"
     ...
     *-------------------------------------------------------
     * All options described below should be passed as a compiler flag to all files using Unity. If
     you must add #defines, place them BEFORE the #include above.

     * Integers/longs/pointers
     *     - Unity attempts to automatically discover your integer sizes
     *       - define UNITY_EXCLUDE_STDINT_H to stop attempting to look in <stdint.h>
     *       - define UNITY_EXCLUDE_LIMITS_H to stop attempting to look in <limits.h>
     *     - If you cannot use the automatic methods above, you can force Unity by using these
     options:
     ...
     *     - define UNITY_EXCLUDE_FLOAT_PRINT to trim binary size, won't print floating point values
     in errors

     * Output
     *     - by default, Unity prints to standard out with putchar.  define UNITY_OUTPUT_CHAR(a)
     with a different function if desired
     *     - define UNITY_DIFFERENTIATE_FINAL_FAIL to print FAILED (vs. FAIL) at test end summary -
     for automated search for failure

     * Optimization
     *     - by default, line numbers are stored in unsigned shorts.  Define UNITY_LINE_TYPE with a
     different type if your files are huge
     *     - by default, test and failure counters are unsigned shorts.  Define UNITY_COUNTER_TYPE
     with a different type if you want to save space or have more than 65535 Tests.

     * Test Cases
     *     - define UNITY_SUPPORT_TEST_CASES to include the TEST_CASE macro, though really it's
     mostly about the runner generator script

     * Parameterized Tests
     *     - you'll want to create a define of TEST_CASE(...), TEST_RANGE(...) and/or
     TEST_MATRIX(...) which basically evaluates to nothing
...
#define TEST_ASSERT_NOT_EQUAL_PTR(expected, actual) \
    UNITY_TEST_ASSERT_NOT_EQUAL_PTR((expected), (actual), __LINE__, NULL)
#define TEST_ASSERT_NOT_EQUAL_STRING(expected, actual) \
    UNITY_TEST_ASSERT_NOT_EQUAL_STRING((expected), (actual), __LINE__, NULL)
#define TEST_ASSERT_NOT_EQUAL_STRING_LEN(expected, actual, len) \
    UNITY_TEST_ASSERT_NOT_EQUAL_STRING_LEN((expected), (actual), (len), __LINE__, NULL)
#define TEST_ASSERT_NOT_EQUAL_MEMORY(expected, actual, len) \
    UNITY_TEST_ASSERT_NOT_EQUAL_MEMORY((expected), (actual), (len), __LINE__, NULL)
...
#define TEST_ASSERT_FLOAT_ARRAY_WITHIN(delta, expected, actual, num_elements) \
    UNITY_TEST_ASSERT_FLOAT_ARRAY_WITHIN(                                     \
        (delta), (expected), (actual), (num_elements), __LINE__, NULL)
...
#define TEST_ASSERT_DOUBLE_ARRAY_WITHIN(delta, expected, actual, num_elements) \
    UNITY_TEST_ASSERT_DOUBLE_ARRAY_WITHIN(                                     \
        (delta), (expected), (actual), (num_elements), __LINE__, NULL)
...
#define TEST_ASSERT_BIT_HIGH_MESSAGE(bit, actual, message) \
    UNITY_TEST_ASSERT_BITS(                                \
        ((UNITY_UINT32)1 << (bit)), (UNITY_UINT32)(-1), (actual), __LINE__, (message))
#define TEST_ASSERT_BIT_LOW_MESSAGE(bit, actual, message) \
    UNITY_TEST_ASSERT_BITS(                               \
        ((UNITY_UINT32)1 << (bit)), (UNITY_UINT32)(0), (actual), __LINE__, (message))
...
/* Structs and Strings Not Equal To */
#define TEST_ASSERT_NOT_EQUAL_PTR_MESSAGE(expected, actual, message) \
    UNITY_TEST_ASSERT_NOT_EQUAL_PTR((expected), (actual), __LINE__, (message))
#define TEST_ASSERT_NOT_EQUAL_STRING_MESSAGE(expected, actual, message) \
    UNITY_TEST_ASSERT_NOT_EQUAL_STRING((expected), (actual), __LINE__, (message))
#define TEST_ASSERT_NOT_EQUAL_STRING_LEN_MESSAGE(expected, actual, len, message) \
    UNITY_TEST_ASSERT_NOT_EQUAL_STRING_LEN((expected), (actual), (len), __LINE__, (message))
#define TEST_ASSERT_NOT_EQUAL_MEMORY_MESSAGE(expected, actual, len, message) \
    UNITY_TEST_ASSERT_NOT_EQUAL_MEMORY((expected), (actual), (len), __LINE__, (message))
