...
protected:
    ...
    void SetUp() override
    {
        ...
        const bal_error_t error
            = bal_x86_assembler_init(&assembler, executable_buffer, sizeof(buffer));
...
{
    EXPECT_EQ(bal_x86_assembler_init(nullptr, executable_buffer, 100), BAL_ERROR_INVALID_ARGUMENT);
}
...
TEST_F(Backendx86Assembler, Init_NullBuffer)
{
    ...
    EXPECT_EQ(bal_x86_assembler_init(&local_assembler, executable_buffer, 100),
...
{
    bal_x86_assembler_t local_assembler;
    EXPECT_EQ(bal_x86_assembler_init(&local_assembler, executable_buffer, 0),
              BAL_ERROR_INVALID_ARGUMENT);
}
...
{
    bal_x86_assembler_t local_assembler;
    EXPECT_EQ(bal_x86_assembler_init(&local_assembler, executable_buffer, 100), BAL_SUCCESS);
...
TEST_F(Backendx86Assembler, Public_NullContext_NoCrash)
{
    ...
    bal_x86_emit_xor_r64_r64(nullptr, BAL_X86_RAX, BAL_X86_RAX);
...
TEST_F(Backendx86Assembler, Public_BadStatus_NoEmit)
{
    ...
    bal_x86_emit_sub_r64_imm32(&assembler, BAL_X86_RAX, 0);
    EXPECT_EQ(assembler.offset, 0);

    bal_x86_emit_sub_r64_r64(&assembler, BAL_X86_RAX, BAL_X86_RBX);
    EXPECT_EQ(assembler.offset, 0);

    bal_x86_emit_test_r64_r64(&assembler, BAL_X86_RAX, BAL_X86_RBX);
    EXPECT_EQ(assembler.offset, 0);

    bal_x86_emit_ret(&assembler);
    EXPECT_EQ(assembler.offset, 0);

    bal_x86_emit_xor_r64_r64(&assembler, BAL_X86_RAX, BAL_X86_RBX);
    EXPECT_EQ(assembler.offset, 0);
...
TEST_F(Backendx86Assembler, Public_InvalidRegister_NoEmit)
{
    ...
    assembler.status = BAL_SUCCESS;
    bal_x86_emit_test_r64_r64(&assembler, bad_register, BAL_X86_RAX);
    EXPECT_EQ(assembler.status, BAL_ERROR_INVALID_ARGUMENT);

    assembler.status = BAL_SUCCESS;
    bal_x86_emit_test_r64_r64(&assembler, BAL_X86_RAX, bad_register);
    EXPECT_EQ(assembler.status, BAL_ERROR_INVALID_ARGUMENT);

    assembler.status = BAL_SUCCESS;
    bal_x86_emit_xor_r64_r64(&assembler, bad_register, BAL_X86_RAX);
    EXPECT_EQ(assembler.status, BAL_ERROR_INVALID_ARGUMENT);

    assembler.status = BAL_SUCCESS;
    bal_x86_emit_xor_r64_r64(&assembler, BAL_X86_RAX, bad_register);
    EXPECT_EQ(assembler.status, BAL_ERROR_INVALID_ARGUMENT);

    assembler.status = BAL_SUCCESS;
    bal_x86_emit_sub_r64_imm32(&assembler, bad_register, 0);
    EXPECT_EQ(assembler.status, BAL_ERROR_INVALID_ARGUMENT);
}
...
TEST_F(Backendx86Assembler, Public_FullBuffer_NoEmit)
{
    ...
    assembler.status = BAL_SUCCESS;
    bal_x86_emit_test_r64_r64(&assembler, BAL_X86_RAX, BAL_X86_RAX);
    EXPECT_EQ(assembler.status, BAL_ERROR_INSTRUCTION_OVERFLOW);
    EXPECT_EQ(assembler.offset, sizeof(buffer));
...
TEST_F(Backendx86Assembler, Encode_SubRegImm_LowReg_8Bit)
{
    bal_x86_emit_sub_r64_imm32(&assembler, BAL_X86_RAX, 0x10);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 4);
    EXPECT_EQ(assembler.buffer[0], 0x48); // REX.W
    EXPECT_EQ(assembler.buffer[1], 0x83); // Opcode
    EXPECT_EQ(assembler.buffer[2], 0xE8); // ModRM (/5, RAX)
    EXPECT_EQ(assembler.buffer[3], 0x10); // Imm8
}

TEST_F(Backendx86Assembler, Encode_SubRegImm_HighReg_32Bit)
{
    bal_x86_emit_sub_r64_imm32(&assembler, BAL_X86_R15, 0x11223344);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 7);
    EXPECT_EQ(assembler.buffer[0], 0x49); // REX.W | REX.B
    EXPECT_EQ(assembler.buffer[1], 0x81); // Opcode
    EXPECT_EQ(assembler.buffer[2], 0xEF); // ModRM (/5, R15)
    EXPECT_EQ(assembler.buffer[3], 0x44); // Imm32 LSB
    EXPECT_EQ(assembler.buffer[4], 0x33);
    EXPECT_EQ(assembler.buffer[5], 0x22);
    EXPECT_EQ(assembler.buffer[6], 0x11); // Imm32 MSB
...
TEST_F(Backendx86Assembler, Encode_SubRegImm_NegativeOffset_SignExtension)
{
    bal_x86_emit_sub_r64_imm32(&assembler, BAL_X86_RAX, -32);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 4);
    EXPECT_EQ(assembler.buffer[0], 0x48); // REX.W
    EXPECT_EQ(assembler.buffer[1], 0x83); // Opcode
    EXPECT_EQ(assembler.buffer[2], 0xE8); // ModRM (/5, RAX)
    EXPECT_EQ(assembler.buffer[3], 0xE0); // Imm8 (-32)
}

TEST_F(Backendx86Assembler, Encode_TestRegToReg_LowLow)
{
    bal_x86_emit_test_r64_r64(&assembler, BAL_X86_RAX, BAL_X86_RBX);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 3);
    EXPECT_EQ(assembler.buffer[0], 0x48); // REX.W
    EXPECT_EQ(assembler.buffer[1], 0x85); // Opcode
    EXPECT_EQ(assembler.buffer[2], 0xD8); // ModRM: 0xC0 | 3 << 3
}

TEST_F(Backendx86Assembler, Encode_TestRegToReg_HighHigh)
{
    bal_x86_emit_test_r64_r64(&assembler, BAL_X86_R14, BAL_X86_R15);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 3);
    EXPECT_EQ(assembler.buffer[0], 0x4D); // REX.W | REX.R | REX.B
    EXPECT_EQ(assembler.buffer[1], 0x85); // Opcode
...
TEST_F(Backendx86Assembler, Encode_XorRegToReg_LowLow)
{
    bal_x86_emit_xor_r64_r64(&assembler, BAL_X86_RAX, BAL_X86_RBX);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 3);
    EXPECT_EQ(assembler.buffer[0], 0x48); // REX.W
    EXPECT_EQ(assembler.buffer[1], 0x33); // Opcode
    EXPECT_EQ(assembler.buffer[2], 0xC3); // ModRM: 0xC0 | 0 << 3 | 3
}

TEST_F(Backendx86Assembler, Encode_XorRegToReg_HighHigh)
{
    bal_x86_emit_xor_r64_r64(&assembler, BAL_X86_R15, BAL_X86_R14);
    EXPECT_EQ(assembler.status, BAL_SUCCESS);
    EXPECT_EQ(assembler.offset, 3);
    EXPECT_EQ(assembler.buffer[0], 0x4D); // REX.W | REX.R | REX.B
    EXPECT_EQ(assembler.buffer[1], 0x33); // Opcode
