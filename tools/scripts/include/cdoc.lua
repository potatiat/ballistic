...
local function main(...)
    ...
    while i <= #args do
        ...
        elseif (args[i] == "--clang-arguments" or args[i] == "--clang-arg") and args[i+1] then
            ...
            i = i + 2
        elseif args[i] == "--theme" and args[i+1] then
            theme_name = args[i+1]
            i = i + 2
        else
    ...
    if #headers == 0 or not color_picker.has(theme_name) then
        print("Usage: luajit cdoc.lua --out-directory <dir> [--theme dark|light] [--clang-library <path>] [--clang-arguments <arg>] [--clang-arg <arg>] <header1.h> ...")
        return 1
    end
