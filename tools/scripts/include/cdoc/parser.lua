    ...
    for _, field_cursor in ipairs(clang.fields(context, tu, record_cursor)) do
        ...
        if name ~= "" then
            fields[#fields + 1] = ast.create_field(
                ...
                clang.field_bit_width(context, field_cursor)
            )
        end
    end
    return fields
...
local function has_body(item)
    return (item.fields and #item.fields > 0) or (item.variants and #item.variants > 0)
end
...
local function add_item(module, seen, item)
    ...
    local existing = seen[item.name]
    if existing then
        -- typedef of an incomplete record can land first; replace it with the
        -- later definition. Keep the first item when both have a body.
        if has_body(item) and not has_body(existing) then
            for index, current in ipairs(module.items) do
        ...
        -- Prefer the documented declaration when a later duplicate has docs
        -- and the first one does not (e.g. struct body then documented typedef).
        if item.documentation and not existing.documentation then
        ...
        if clang.from_main_file(context, cursor) then
            ...
            elseif kind == clang.CURSOR.TypedefDecl then
                ...
                elseif type_kind == clang.TYPE.Enum then
                    ...
                    local parameters = {}
                    -- Use canonical so `typedef existing_fn_ptr_t alias_t` still
                    -- sees Pointer+FunctionProto when underlying is a typedef.
                    if clang.type_kind(canonical) == clang.TYPE.Pointer then
                        local pointee = clang.pointee_type(context, canonical)
                        if clang.type_kind(pointee) == clang.TYPE.FunctionProto then
                            ...
                            -- TypedefDecl ParmDecl children carry names; fall back
                            -- to the prototype for typedef-of-typedef aliases.
            ...
            elseif kind == clang.CURSOR.MacroDefinition then
                if not is_include_guard(name, module.name) then
                    local value, function_like = clang.macro_replacement(context, tu, cursor)
...
function M.parse_header(clang_context, header_path, clang_args)
    ...
    local filled, result = pcall(function()
        ...
        if not module_name:match("^[%w._-]+%.h$") then
            error("header basename '" .. module_name .. "' is not a safe module name.", 0)
        end
    ...
    if not filled then
        log.error("Aborting function: failed to walk '%s' because %s.", header_path, tostring(result))
        return nil
    end
    return result
end
