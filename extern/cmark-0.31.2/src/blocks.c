...
static bool
S_last_line_blank(const cmark_node *node)
{
    return (node->flags & CMARK_NODE__LAST_LINE_BLANK) != 0;
}

static bool
S_last_line_checked(const cmark_node *node)
{
    return (node->flags & CMARK_NODE__LAST_LINE_CHECKED) != 0;
}

static inline cmark_node_type
S_type(const cmark_node *node)
{
...
static void
S_set_last_line_checked(cmark_node *node)
{
...
static inline bool
S_is_space_or_tab(char c)
{
    return (c == ' ' || c == '\t');
}

static void S_parser_feed(cmark_parser *parser, const unsigned char *buffer, size_t len, bool eof);

static void S_process_line(cmark_parser *parser, const unsigned char *buffer, bufsize_t bytes);

static cmark_node *
make_block(cmark_mem *mem, cmark_node_type tag, int start_line, int start_column)
{
    cmark_node *e;
...
cmark_parser *
cmark_parser_new_with_mem_into_root(int options, cmark_mem *mem, cmark_node *root)
{
...
cmark_parser *
cmark_parser_new_with_mem(int options, cmark_mem *mem)
{
...
void
cmark_parser_free(cmark_parser *parser)
{
...
static bool
is_blank(cmark_strbuf *s, bufsize_t offset)
{
    while (offset < s->size)
    {
        switch (s->ptr[offset])
...
static inline bool
can_contain(cmark_node_type parent_type, cmark_node_type child_type)
{
    return (parent_type == CMARK_NODE_DOCUMENT || parent_type == CMARK_NODE_BLOCK_QUOTE
            || parent_type == CMARK_NODE_ITEM
            || (parent_type == CMARK_NODE_LIST && child_type == CMARK_NODE_ITEM));
}

static inline bool
accepts_lines(cmark_node_type block_type)
{
...
static inline bool
contains_inlines(cmark_node_type block_type)
{
    return (block_type == CMARK_NODE_PARAGRAPH || block_type == CMARK_NODE_HEADING);
}

static void
add_line(cmark_chunk *ch, cmark_parser *parser)
{
    int chars_to_tab;
    int i;
...
static void
remove_trailing_blank_lines(cmark_strbuf *ln)
{
    bufsize_t     i;
    unsigned char c;
...
static bool
S_ends_with_blank_line(cmark_node *node)
{
    if (S_last_line_checked(node))
    {
        return (S_last_line_blank(node));
    }
    else if ((S_type(node) == CMARK_NODE_LIST || S_type(node) == CMARK_NODE_ITEM)
             && node->last_child)
    {
...
static bool
resolve_reference_link_definitions(cmark_parser *parser)
{
    bufsize_t     pos;
...
static cmark_node *
add_child(cmark_parser *parser, cmark_node *parent, cmark_node_type block_type, int start_column)
{
    assert(parent);
...
static void
process_inlines(cmark_mem *mem, cmark_node *root, cmark_reference_map *refmap, int options)
{
...
static bufsize_t
parse_list_marker(cmark_mem   *mem,
                  cmark_chunk *input,
                  bufsize_t    pos,
...
static int
lists_match(cmark_list *list_data, cmark_list *item_data)
{
...
static cmark_node *
finalize_document(cmark_parser *parser)
{
...
cmark_node *
cmark_parse_file(FILE *f, int options)
{
...
cmark_node *
cmark_parse_document(const char *buffer, size_t len, int options)
{
    cmark_parser *parser = cmark_parser_new(options);
    cmark_node   *document;
...
static void
S_parser_feed(cmark_parser *parser, const unsigned char *buffer, size_t len, bool eof)
{
...
static void
chop_trailing_hashtags(cmark_chunk *ch)
{
    bufsize_t n, orig_n;
...
static int
S_scan_thematic_break(cmark_parser *parser, cmark_chunk *input, bufsize_t offset)
{
    bufsize_t i;
    char      c;
    char      nextc = '\0';
...
static bool
S_last_child_is_open(cmark_node *container)
{
    return container->last_child && (container->last_child->flags & CMARK_NODE__OPEN);
}

static bool
parse_block_quote_prefix(cmark_parser *parser, cmark_chunk *input)
{
    bool      res     = false;
    bufsize_t matched = 0;
...
static bool
parse_node_item_prefix(cmark_parser *parser, cmark_chunk *input, cmark_node *container)
{
    bool res = false;
    ...
    {
        S_advance_offset(
            parser, input, container->as.list.marker_offset + container->as.list.padding, true);
        res = true;
    }
...
static bool
parse_code_block_prefix(cmark_parser *parser,
                        cmark_chunk  *input,
...
static bool
parse_html_block_prefix(cmark_parser *parser, cmark_node *container)
{
    bool res             = false;
 ...
 */
static cmark_node *
check_open_blocks(cmark_parser *parser, cmark_chunk *input, bool *all_matched)
{
    ...
    while (S_last_child_is_open(container))
    {
        ...
        switch (cont_type)
        {
            ...
            case CMARK_NODE_LIST:
                ...
                if (parser->blank)
                {
                    if (container->flags & CMARK_NODE__LIST_LAST_LINE_BLANK && parser->indent == 0)
                    {
...
static void
open_new_blocks(cmark_parser *parser, cmark_node **container, cmark_chunk *input, bool all_matched)
{
    bool            indented;
    ...
    while (cont_type != CMARK_NODE_CODE_BLOCK && cont_type != CMARK_NODE_HTML_BLOCK)
    {
        ...
        else if (!indented && (matched = scan_atx_heading_start(input, parser->first_nonspace)))
        {
            ...
            S_advance_offset(
                parser, input, parser->first_nonspace + matched - parser->offset, false);
            ...
            S_advance_offset(
                parser, input, parser->first_nonspace + matched - parser->offset, false);
                 ...
                 && ((matched = scan_html_block_start(input, parser->first_nonspace))
                     || (cont_type != CMARK_NODE_PARAGRAPH && !maybe_lazy
        ...
        else if (!indented && !(cont_type == CMARK_NODE_PARAGRAPH && !all_matched)
                 && (parser->thematic_break_kill_pos <= parser->first_nonspace)
                 && S_scan_thematic_break(parser, input, parser->first_nonspace))
        {
            // it's only now that we know the line is not part of a setext heading:
            *container = add_child(
                parser, *container, CMARK_NODE_THEMATIC_BREAK, parser->first_nonspace + 1);
            S_advance_offset(parser, input, input->len - 1 - parser->offset, false);
        }
        else if ((!indented || cont_type == CMARK_NODE_LIST) && parser->indent < 4
                 && (matched = parse_list_marker(parser->mem,
                                                 input,
                                                 ...
                                                 &data)))
        {
            ...
            if (cont_type != CMARK_NODE_LIST || !lists_match(&((*container)->as.list), data))
            {
                *container
...
static void
add_text_to_container(cmark_parser *parser,
                      cmark_node   *container,
