...
static void
escape_html(cmark_strbuf *dest, const unsigned char *source, bufsize_t length)
{
...
static int
S_render_node(cmark_node *node, cmark_event_type ev_type, struct render_state *state, int options)
{
    cmark_node   *parent;
    ...
    switch (node->type)
    {
        ...
        case CMARK_NODE_LIST: {
            ...
            {
                cmark_strbuf_puts(html, list_type == CMARK_BULLET_LIST ? "</ul>\n" : "</ol>\n");
            }
            break;
        }
        ...
        case CMARK_NODE_CUSTOM_BLOCK: {
            unsigned char *block = entering ? node->as.custom.on_enter : node->as.custom.on_exit;
            cr(html);
            if (block)
            {
        ...
        case CMARK_NODE_LINK:
            if (entering)
            {
                ...
                if (node->as.link.url
                    && ((options & CMARK_OPT_UNSAFE) || !(_scan_dangerous_url(node->as.link.url))))
                {
                    houdini_escape_href(
                        html, node->as.link.url, (bufsize_t)strlen((char *)node->as.link.url));
                }
                if (node->as.link.title)
                {
                    cmark_strbuf_puts(html, "\" title=\"");
                    escape_html(
                        html, node->as.link.title, (bufsize_t)strlen((char *)node->as.link.title));
                }
                cmark_strbuf_puts(html, "\">");
            }
            else
...
char *
cmark_render_html(cmark_node *root, int options)
{
    char               *result;
