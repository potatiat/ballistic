...
void
cmark_strbuf_init(cmark_mem *mem, cmark_strbuf *buf, bufsize_t initial_size)
{
    buf->mem   = mem;
    buf->asize = 0;
    buf->size  = 0;
...
static inline void
S_strbuf_grow_by(cmark_strbuf *buf, bufsize_t add)
{
    cmark_strbuf_grow(buf, buf->size + add);
}

void
cmark_strbuf_grow(cmark_strbuf *buf, bufsize_t target_size)
{
    ...
    buf->ptr   = (unsigned char *)buf->mem->realloc(buf->asize ? buf->ptr : NULL, new_size);
    buf->asize = new_size;
}
...
void
cmark_strbuf_clear(cmark_strbuf *buf)
{
    buf->size = 0;
...
void
cmark_strbuf_set(cmark_strbuf *buf, const unsigned char *data, bufsize_t len)
{
    if (len <= 0 || data == NULL)
    {
...
void
cmark_strbuf_putc(cmark_strbuf *buf, int c)
{
...
void
cmark_strbuf_puts(cmark_strbuf *buf, const char *string)
{
    cmark_strbuf_put(buf, (const unsigned char *)string, (bufsize_t)strlen(string));
}

unsigned char *
cmark_strbuf_detach(cmark_strbuf *buf)
{
...
void
cmark_strbuf_trim(cmark_strbuf *buf)
{
    bufsize_t i = 0;
...
void
cmark_strbuf_normalize_whitespace(cmark_strbuf *s)
{
...
void
cmark_strbuf_unescape(cmark_strbuf *buf)
{
    bufsize_t r, w;
