...
static void
S_outc(cmark_renderer *renderer, cmark_escaping escape, int32_t c, unsigned char nextc)
{
    (void)(nextc);
...
S_render_node(cmark_renderer *renderer, cmark_node *node, cmark_event_type ev_type, int options)
{
    ...
    if (renderer->block_number_in_list_item && node->type < CMARK_NODE_FIRST_INLINE)
    {
        if (entering)
        {
    ...
    switch (node->type)
    {
        ...
        case CMARK_NODE_PARAGRAPH:
            if (entering)
            {
                ...
                if (node->parent && node->parent->type == CMARK_NODE_ITEM && node->prev == NULL)
                {
                    // no blank line or .PP
                }
                else
...
char *
cmark_render_man(cmark_node *root, int options, int width)
{
