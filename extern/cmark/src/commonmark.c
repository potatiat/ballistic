...
static inline void
outc(cmark_renderer *renderer, cmark_escaping escape, int32_t c, unsigned char nextc)
{
    ...
    needs_escaping
        = c < 0x80 && escape != LITERAL
          && ((escape == NORMAL
              ...
              || (escape == URL
                  && (c == '`' || c == '<' || c == '>' || cmark_isspace(c) || c == '\\' || c == ')'
                      || c == '('))
              || (escape == TITLE && (c == '`' || c == '<' || c == '>' || c == '"' || c == '\\')));
...
is_autolink(cmark_node *node)
{
    ...
    return link_text->data != NULL && strcmp((const char *)url, (char *)link_text->data) == 0;
}

static int
S_render_node(cmark_renderer *renderer, cmark_node *node, cmark_event_type ev_type, int options)
{
    cmark_node      *tmp;
    int              list_number;
    ...
    else
    {
        ...
        {
            renderer->in_tight_list_item = node->parent && node->parent->type == CMARK_NODE_ITEM
    ...
    switch (node->type)
    {
        ...
        case CMARK_NODE_ITEM:
            ...
            {
                cmark_strbuf_truncate(renderer->prefix, renderer->prefix->size - marker_width);
                CR();
            }
            break;
        ...
        case CMARK_NODE_CODE_BLOCK:
            ...
            first_in_list_item
                = node->prev == NULL && node->parent && node->parent->type == CMARK_NODE_ITEM;
        ...
        case CMARK_NODE_SOFTBREAK:
            if (CMARK_OPT_HARDBREAKS & options)
            {
                ...
                CR();
                     ...
                     && !(CMARK_OPT_HARDBREAKS & options) && !(CMARK_OPT_NOBREAKS & options))
            {
                CR();
            }
            else
        ...
        case CMARK_NODE_CODE:
            ...
            extra_spaces = code_len == 0 || code[0] == '`' || code[code_len - 1] == '`'
        ...
        case CMARK_NODE_EMPH:
            ...
            if (node->parent && node->parent->type == CMARK_NODE_EMPH && node->next == NULL
                && node->prev == NULL)
            {
                emph_delim = "_";
            }
...
char *
cmark_render_commonmark(cmark_node *root, int options, int width)
{
