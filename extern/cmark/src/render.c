...
static inline void
S_blankline(cmark_renderer *renderer)
{
...
static void
S_out(cmark_renderer *renderer, const char *source, bool wrap, cmark_escaping escape)
{
    ...
    while (renderer->need_cr)
    {
        ...
        else
        {
            ...
            if (renderer->need_cr > 1)
            {
                cmark_strbuf_put(renderer->buffer, renderer->prefix->ptr, renderer->prefix->size);
            }
        }
        renderer->column         = 0;
    ...
    while (i < length)
    {
        ...
        else if (escape == LITERAL)
        {
            ...
            else
            {
                ...
                // a digit:
                renderer->begin_content = renderer->begin_content && cmark_isdigit(c) == 1;
            }
        }
        else
        {
            ...
            && renderer->last_breakable > 0)
        {
            ...
            unsigned char *src           = renderer->buffer->ptr + renderer->last_breakable + 1;
            bufsize_t      remainder_len = renderer->buffer->size - renderer->last_breakable - 1;
            unsigned char *remainder = (unsigned char *)renderer->mem->realloc(NULL, remainder_len);
...
cmark_render(cmark_node *root,
             ...
             int (*render_node)(
                 cmark_renderer *renderer, cmark_node *node, cmark_event_type ev_type, int options))
{
    ...
    cmark_renderer renderer = { options, mem,   &buf,  &pref, 0,    width, 0,           0,    true,
                                true,    false, false, NULL,  outc, S_cr,  S_blankline, S_out };
