...
bool
cmark_node_is_block(cmark_node *node)
{
    if (node == NULL)
    {
        return false;
...
bool
cmark_node_is_inline(cmark_node *node)
{
    if (node == NULL)
    {
        return false;
    }
    return node->type >= CMARK_NODE_FIRST_INLINE && node->type <= CMARK_NODE_LAST_INLINE;
}

bool
cmark_node_is_leaf(cmark_node *node)
{
    if (node == NULL)
    {
        return false;
...
cmark_node *
cmark_node_new_with_mem(cmark_node_type type, cmark_mem *mem)
{
...
static bufsize_t
cmark_set_cstr(cmark_mem *mem, unsigned char **dst, const char *src)
{
...
int
cmark_node_get_heading_level(cmark_node *node)
{
    if (node == NULL)
    {
        return 0;
...
int
cmark_node_set_heading_level(cmark_node *node, int level)
{
...
const char *
cmark_node_get_on_enter(cmark_node *node)
{
    if (node == NULL)
    {
        return NULL;
...
int
cmark_node_set_on_enter(cmark_node *node, const char *on_enter)
{
    if (node == NULL)
    {
        return 0;
...
const char *
cmark_node_get_on_exit(cmark_node *node)
{
    if (node == NULL)
    {
        return NULL;
...
int
cmark_node_set_on_exit(cmark_node *node, const char *on_exit)
{
    if (node == NULL)
    {
        return 0;
...
int
cmark_node_replace(cmark_node *oldnode, cmark_node *newnode)
{
