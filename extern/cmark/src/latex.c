...
get_link_type(cmark_node *node)
{
    ...
    if (title_len == 0)
    {
        ...
        if (realurllen == link_text->len
            && strncmp(realurl, (char *)link_text->data, link_text->len) == 0)
        {
            if (isemail)
            {
...
S_get_enumlevel(cmark_node *node)
{
    ...
    while (tmp)
    {
        if (tmp->type == CMARK_NODE_LIST && cmark_node_get_list_type(node) == CMARK_ORDERED_LIST)
        {
            enumlevel++;
        }
        tmp = tmp->parent;
        ...
        case CMARK_NODE_LIST:
            ...
            if (entering)
            {
                ...
                if (list_number > 1)
                {
                    ...
                    {
                        snprintf(list_number_string,
                                 LIST_NUMBER_STRING_SIZE,
                                 "%d",
...
char *
cmark_render_latex(cmark_node *root, int options, int width)
{
