...
static void test_md_to_html(test_batch_runner *runner,
                            const char        *markdown,
                            const char        *expected_html,
                            const char        *msg);

static void test_content(test_batch_runner *runner, cmark_node_type type, int allowed_content);
...
version(test_batch_runner *runner)
{
    ...
    STR_EQ(runner, cmark_version_string(), CMARK_VERSION_STRING, "cmark_version_string");
}
    ...
    for (int i = 0; i < num_node_types; ++i)
    {
        ...
        switch (node->type)
        {
            case CMARK_NODE_HEADING:
                INT_EQ(runner, cmark_node_get_heading_level(node), 1, "default heading level is 1");
            ...
            case CMARK_NODE_LIST:
                ...
                INT_EQ(runner,
                       cmark_node_get_list_delim(node),
                       CMARK_NO_DELIM,
                       "default is list delim is NO_DELIM");
                INT_EQ(runner, cmark_node_get_list_start(node), 0, "default is list start is 0");
                INT_EQ(runner, cmark_node_get_list_tight(node), 0, "default is list is loose");
                break;
...
static void
classifiers(test_batch_runner *runner)
{
...
accessors(test_batch_runner *runner)
{
    ...
    INT_EQ(
        runner, cmark_node_get_list_type(bullet_list), CMARK_BULLET_LIST, "get_list_type bullet");
    INT_EQ(runner, cmark_node_get_list_tight(bullet_list), 1, "get_list_tight tight");

    cmark_node *ordered_list = cmark_node_next(bullet_list);
    INT_EQ(runner,
           cmark_node_get_list_type(ordered_list),
           CMARK_ORDERED_LIST,
           "get_list_type ordered");
    INT_EQ(runner,
           cmark_node_get_list_delim(ordered_list),
           CMARK_PERIOD_DELIM,
           "get_list_delim ordered");
    INT_EQ(runner, cmark_node_get_list_start(ordered_list), 2, "get_list_start");
    INT_EQ(runner, cmark_node_get_list_tight(ordered_list), 0, "get_list_tight loose");
    ...
    OK(runner, cmark_node_set_list_type(bullet_list, CMARK_ORDERED_LIST), "set_list_type ordered");
    OK(runner, cmark_node_set_list_delim(bullet_list, CMARK_PAREN_DELIM), "set_list_delim paren");
    ...
    OK(runner, cmark_node_set_list_type(ordered_list, CMARK_BULLET_LIST), "set_list_type bullet");
    OK(runner, cmark_node_set_list_tight(ordered_list, 1), "set_list_tight tight");
    ...
    OK(runner, cmark_node_set_literal(string, "prefix-LINK"), "set_literal string");

    // Set literal to suffix of itself (issue #139).
    const char *literal = cmark_node_get_literal(string);
    OK(runner, cmark_node_set_literal(string, literal + sizeof("prefix")), "set_literal suffix");

    char             *rendered_html = cmark_render_html(doc, CMARK_OPT_DEFAULT | CMARK_OPT_UNSAFE);
    ...
    INT_EQ(runner, cmark_node_get_heading_level(bullet_list), 0, "get_heading_level error");
    INT_EQ(runner, cmark_node_get_list_type(heading), CMARK_NO_LIST, "get_list_type error");
    ...
    OK(runner, cmark_node_get_fence_info(paragraph) == NULL, "get_fence_info error");
    OK(runner, cmark_node_get_url(html) == NULL, "get_url error");
    OK(runner, cmark_node_get_title(heading) == NULL, "get_title error");

    // Setter errors

    OK(runner, !cmark_node_set_heading_level(bullet_list, 3), "set_heading_level error");
    OK(runner, !cmark_node_set_list_type(heading, CMARK_ORDERED_LIST), "set_list_type error");
    ...
    OK(runner, !cmark_node_set_fence_info(paragraph, "lang"), "set_fence_info error");
    OK(runner, !cmark_node_set_url(html, "url"), "set_url error");
    OK(runner, !cmark_node_set_title(heading, "title"), "set_title error");

    OK(runner, !cmark_node_set_heading_level(heading, 0), "set_heading_level too small");
    OK(runner, !cmark_node_set_heading_level(heading, 7), "set_heading_level too large");
    OK(runner, !cmark_node_set_list_type(bullet_list, CMARK_NO_LIST), "set_list_type invalid");
    OK(runner, !cmark_node_set_list_start(bullet_list, -1), "set_list_start negative");

    cmark_node_free(doc);
}

static void
free_parent(test_batch_runner *runner)
{
    ...
    cmark_node_free(doc);
    STR_EQ(
        runner, cmark_node_get_literal(text), "text", "inline content after freeing parent block");
...
hierarchy(test_batch_runner *runner)
{
    ...
    OK(runner,
       !cmark_node_append_child(bquote3, bquote3),
       "adding a node as child of itself fails");
    OK(runner, !cmark_node_append_child(bquote3, bquote1), "adding a parent as child fails");

    cmark_node_free(bquote1);

    int max_node_type = CMARK_NODE_LAST_BLOCK > CMARK_NODE_LAST_INLINE ? CMARK_NODE_LAST_BLOCK
                                                                       : CMARK_NODE_LAST_INLINE;
    OK(runner, max_node_type < 32, "all node types < 32");

    int list_item_flag   = 1 << CMARK_NODE_ITEM;
    int top_level_blocks = (1 << CMARK_NODE_BLOCK_QUOTE) | (1 << CMARK_NODE_LIST)
    ...
    int all_inlines = (1 << CMARK_NODE_TEXT) | (1 << CMARK_NODE_SOFTBREAK)
                      ...
                      | (1 << CMARK_NODE_STRONG) | (1 << CMARK_NODE_LINK) | (1 << CMARK_NODE_IMAGE);
...
static void
test_content(test_batch_runner *runner, cmark_node_type type, int allowed_content)
{
    cmark_node *node = cmark_node_new(type);
...
static void
render_xml(test_batch_runner *runner)
{
    char *xml;
    ...
    STR_EQ(runner,
           ...
           "  </paragraph>\n"
           "  <paragraph>\n"
           "    <text xml:space=\"preserve\">control -" UTF8_REPL
           "-</text>\n"
           "    <softbreak />\n"
           "    <text xml:space=\"preserve\">fffe -" UTF8_REPL
           "-</text>\n"
           "    <softbreak />\n"
           "    <text xml:space=\"preserve\">ffff -" UTF8_REPL
           "-</text>\n"
           "    <softbreak />\n"
...
static void
render_man(test_batch_runner *runner)
{
    char *man;
...
static void
render_latex(test_batch_runner *runner)
{
    char *latex;
...
static void
render_commonmark(test_batch_runner *runner)
{
    char *commonmark;
...
utf8(test_batch_runner *runner)
{
    ...
    char             *html
        = cmark_markdown_to_html(string_with_null, sizeof(string_with_null) - 1, CMARK_OPT_DEFAULT);
...
static void
test_char(test_batch_runner *runner, int valid, const char *utf8, const char *msg)
{
    char buf[20];
    sprintf(buf, "((((%s))))", utf8);
...
static void
test_incomplete_char(test_batch_runner *runner, const char *utf8, const char *msg)
{
    char buf[20];
    sprintf(buf, "----%s", utf8);
    test_md_to_html(runner, buf, "<p>----" UTF8_REPL "</p>\n", msg);
...
test_continuation_byte(test_batch_runner *runner, const char *utf8)
{
    ...
    for (size_t pos = 1; pos < len; ++pos)
    {
        ...
        char *html = cmark_markdown_to_html(buf, strlen(buf), CMARK_OPT_VALIDATE_UTF8);
        STR_EQ(runner, html, expected, "invalid utf8 continuation byte %d/%d", pos, len);
        free(html);
    }
}
...
line_endings(test_batch_runner *runner)
{
    ...
    html                           = cmark_markdown_to_html(
        crlf_lines, sizeof(crlf_lines) - 1, CMARK_OPT_DEFAULT | CMARK_OPT_HARDBREAKS);
    STR_EQ(runner, html, "<p>line<br />\nline</p>\n", "crlf endings with CMARK_OPT_HARDBREAKS");
    free(html);
    html = cmark_markdown_to_html(
        crlf_lines, sizeof(crlf_lines) - 1, CMARK_OPT_DEFAULT | CMARK_OPT_NOBREAKS);
    STR_EQ(runner, html, "<p>line line</p>\n", "crlf endings with CMARK_OPT_NOBREAKS");
    free(html);

    static const char no_line_ending[] = "```\nline\n```";
    html = cmark_markdown_to_html(no_line_ending, sizeof(no_line_ending) - 1, CMARK_OPT_DEFAULT);
    STR_EQ(runner,
           html,
           "<pre><code>line\n</code></pre>\n",
...
test_safe(test_batch_runner *runner)
{
    ...
    static const char raw_html[]
        = "<div>\nhi\n</div>\n\n<a>hi</"
          ...
          "file:my.js)\n";
    char *html = cmark_markdown_to_html(raw_html, sizeof(raw_html) - 1, CMARK_OPT_DEFAULT);
    STR_EQ(runner,
           html,
           "<!-- raw HTML omitted -->\n<p><!-- raw HTML omitted "
           "-->hi<!-- raw HTML omitted -->\n<a "
...
static void
test_md_to_html(test_batch_runner *runner,
                const char        *markdown,
                const char        *expected_html,
                const char        *msg)
{
    char *html = cmark_markdown_to_html(markdown, strlen(markdown), CMARK_OPT_VALIDATE_UTF8);
    STR_EQ(runner, html, expected_html, msg);
    free(html);
}

static void
test_feed_across_line_ending(test_batch_runner *runner)
{
    // See #117
...
static void
sub_document(test_batch_runner *runner)
{
    cmark_node *doc  = cmark_node_new(CMARK_NODE_DOCUMENT);
...

    {
        ...
        static const char markdown[] = "Hello &ldquo; <http://www.google.com>\n";
        cmark_parser     *parser     = cmark_parser_new_with_mem_into_root(
            CMARK_OPT_DEFAULT, cmark_get_default_mem_allocator(), item);
...

    {
        cmark_node *item = cmark_node_new(CMARK_NODE_ITEM);
        OK(runner, cmark_node_append_child(list, item), "append_0");
        static const char markdown[] = "Bye &ldquo; <http://www.geocities.com>\n";
        cmark_parser     *parser     = cmark_parser_new_with_mem_into_root(
            CMARK_OPT_DEFAULT, cmark_get_default_mem_allocator(), item);
        cmark_parser_feed(parser, markdown, sizeof(markdown) - 1);
        OK(runner, cmark_parser_finish(parser) != NULL, "parser_finish_0");
...
static void
source_pos(test_batch_runner *runner)
{
    static const char markdown[]
        = "# Hi **there*.\n"
          "\n"
    ...
    STR_EQ(
        ...
        "    <list sourcepos=\"6:3-10:20\" type=\"ordered\" start=\"1\" delim=\"period\" "
        "tight=\"false\">\n"
...
static void
source_pos_inlines(test_batch_runner *runner)
{
    {
        static const char markdown[]
            = "*first*\n"
    ...
    }
    {
        ...
        STR_EQ(
            ...
            "    <softbreak />\n"
            "    <text sourcepos=\"2:1-2:25\" xml:space=\"preserve\">`` They are two "
            "backticks</text>\n"
            "  </paragraph>\n"
            "</document>\n",
            "sourcepos are as expected");
...
static void
ref_source_pos(test_batch_runner *runner)
{
    static const char markdown[]
