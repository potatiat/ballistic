...
local map_global = setmetatable({}, { __index = function(t, name)
    if not match(name, "^[%a_][%w_@]*$") then
        werror("bad global label")
    end
    local n = next_global
    if n > 246 then
        werror("too many global labels")
    end
    next_global = n + 1
    t[name] = n
    return n
...
local function dumpglobals(out, lvl)
    local t = {}
    for name, n in pairs(map_global) do
        t[n] = name
    end
...
local function dumpexterns(out, lvl)
    local t = {}
    for name, n in pairs(map_extern) do
        t[-n] = name
    end
...
local function mkrmap(sz, cl, names)
    ...
    if names then
        for n, name in ipairs(names) do
            ...
            if sz == "b" and n > 4 then
                map_reg_needrex[iname] = false
            end
            if sz == addrsize or sz == "d" then
    ...
    for i = 0, (x64 and sz ~= "f") and 15 or 7 do
        ...
        if needrex then
            map_reg_needrex[iname] = true
        end
        local name
        if sz == "o" or sz == "y" then
            name = format("%s%d", cl, i)
        elseif sz == "f" then
            name = format("st%d", i)
        else
            name = format("r%d%s", i, sz == addrsize and "" or sz)
        end
        map_archdef[name] = iname
...
if x64 then
    map_reg_valid_index[map_archdef.rsp] = false
end
if x64 then
    map_reg_needrex[map_archdef.Rb] = true
end
...
local function wputqarg(n)
    local tn = type(n)
    if tn == "number" then
        -- This is only used for numbers from -2^31..2^32-1.
        wputb(band(n, 255))
        wputb(band(shr(n, 8), 255))
        wputb(band(shr(n, 16), 255))
        wputb(shr(n, 24))
...
local function wputszarg(sz, n)
    if not sz or sz == "d" or sz == "q" then
        wputdarg(n)
    elseif sz == "w" then
        wputwarg(n)
    elseif sz == "b" then
...
local function wputop(sz, op, rex, vex, vregr, vregxb)
    ...
    if op >= 4294967296 then
        r = op % 4294967296
        wputb((op - r) / 4294967296)
        op = r
    end
    if op >= 16777216 then
        wputb(shr(op, 24));
...
local function wputmrmsib(t, imark, s, vsreg, psz, sk)
    ...
    if not reg then
        ...
        if riprel then
            -- Emit rip-relative displacement.
    ...
    local m
    if tdisp == "number" then
        -- Check displacement size at assembly time.
        if disp == 0 and band(reg, 7) ~= 5 then
            -- [ebp] -> [ebp+0] (in SIB, too)
            if not vreg then
                m = 0
            end -- Force DISP to allow [Rd(5)] -> [ebp+0]
...
local function dispexpr(expr)
    ...
    if mode == "iJ" or mode == "iPJ" then
        if c == "-" then
            werror("cannot invert label reference")
        end
        return { iexpr, mode }
    end
    ...
    repeat
        if br then
            ...
            if t.reg == -1 then
                ...
                if not t.vreg then
                    werror("bad variable register expression")
                end
            end
            ...
            if imm then
                ...
                if imm >= 4294967168 and imm <= 4294967295 then
                    imm = imm - 4294967296
                end
...
local function dopattern(pat, args, sz, op, needrex)
    ...
    for c in gmatch(pat .. "|", ".") do
        if match(c, "%x") then
            ...
            addin = nil
        elseif c == "n" then
            -- Disable operand size mods for opcode.
            szov = nil
        elseif c == "X" then
            -- Force REX.W.
            rex = 8
        elseif c == "L" then
            ...
            vex.l = true
        elseif c == "r" then
            -- Merge 1st operand regno. into opcode.
            addin = args[1];
        ...
        elseif c == "R" then
            -- Merge 2nd operand regno. into opcode.
            addin = args[2];
            opcode = opcode + (addin.reg % 8)
            narg = 3
        elseif c == "m" or c == "M" then
            ...
            if szov == "q" and rex == 0 then
                rex = rex + 8
            end
            if t.reg and t.reg > 7 then
                rex = rex + 1
            end
            if t.xreg and t.xreg > 7 then
                rex = rex + 2
            end
            if s > 7 then
                rex = rex + 4
            ...
            addin = nil
        elseif map_vexarg[c] ~= nil then
            -- Encode using VEX prefix
            ...
            if c == "o" then
                -- Offset (pure 32 bit displacement).
...
local function matchtm(tm, args)
    for i = 1, #args do
        if not match(args[i].mode, sub(tm, i, i)) then
            return
        end
    end
    return true
...
map_op[".template__"] = function(params, template, nparams)
    if not params then
        return templatehelp(template, nparams)
    end
    local args = {}
    ...
    for tm in gmatch(template, "[^%|]+") do
        ...
        if matchtm(tm, args) then
            ...
            if prefix == "/" then
                -- Exactly match leading operand sizes.
                for i = #szm, 1, -1 do
                    if i == 1 then
                ...
                -- Match common operand size.
                local szp = sz
                if szm == "" then
                    szm = x64 and "qdwb" or "dwb"
                end -- Default sizes.
                if prefix == "1" then
                    szp = args[1].opsize;
                    szmix = nil
...
if x64 then
    function map_op.mov64_2(params)
        if not params then
            return { "reg, imm", "reg, [disp]", "[disp], reg" }
        end
        if secpos + 2 > maxsecpos then
            wflush()
        end
...
map_op[".space_2"] = function(params)
    ...
    if fill then
        fill = tonumber(fill)
        if not fill or fill < 0 or fill > 255 then
            werror("bad filler")
        end
    end
    wputxb(fill or 0)
