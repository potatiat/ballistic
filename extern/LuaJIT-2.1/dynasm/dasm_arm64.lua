...
local function parse_extend(expr)
    ...
    if not s then
        werror("expected extend operand")
    end
...
local function parse_cond(expr, inv)
    ...
    if not c then
        werror("expected condition operand")
    end
        ...
        if p2a ~= "" then
            ...
            if imm then
                ...
                if p3b == "" then
                    ...
                    if parse_reg_type == "x" then
                        if p3b == "lsl" and p3s ~= "" then
                            op = op + 0x6000
                        elseif p3b == "sxtx" then
                            op = op + 0xe000
                        else
...
local function branch_type(op)
    if band(op, 0x7c000000) == 0x14000000 then
        return 0 -- B, BL
    ...
    elseif band(op, 0x7e000000) == 0x36000000 then
        return 0x1000 -- TBZ, TBNZ
    elseif band(op, 0x9f000000) == 0x10000000 then
        return 0x2000 -- ADR
    elseif band(op, 0x9f000000) == band(0x90000000) then
        return 0x3000 -- ADRP
    else
...
local function op_data(params)
    ...
    for _, p in ipairs(params) do
        ...
        if not imm then
            ...
            if sz == 4 then
                if mode then
                    werror("label does not fit into .long")
                end
                waction("IMMV", 0, p)
