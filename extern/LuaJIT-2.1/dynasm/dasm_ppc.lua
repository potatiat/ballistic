...
map_op = {
    ...
    subi_3 = op_alias("addi_3", function(p)
        p[3] = "-(" .. p[3] .. ")"
    end),
    subis_3 = op_alias("addis_3", function(p)
        p[3] = "-(" .. p[3] .. ")"
    end),
    subic_3 = op_alias("addic_3", function(p)
        p[3] = "-(" .. p[3] .. ")"
    end),
    ["subic._3"] = op_alias("addic._3", function(p)
        p[3] = "-(" .. p[3] .. ")"
    end),
        ...
        if shl(m, scale) == n then
            if signed then
                ...
                if s == 0 then
                    return shl(m, shift)
                elseif s == -1 then
                    return shl(m + shl(1, bits), shift)
                end
            else
                if sar(m, bits) == 0 then
                    return shl(m, shift)
                end
            end
        end
...
local function parse_disp(disp)
    ...
    if imm then
        ...
        if r == 0 then
            werror("cannot use r0 in displacement")
        end
...
op_template = function(params, template, nparams)
    ...
    for p in gmatch(sub(template, 9), ".") do
        if p == "R" then
            rs = rs - 5;
            op = op + shl(parse_gpr(params[n]), rs);
            n = n + 1
        elseif p == "F" then
            rs = rs - 5;
            op = op + shl(parse_fpr(params[n]), rs);
            n = n + 1
        elseif p == "V" then
            rs = rs - 5;
            op = op + shl(parse_vr(params[n]), rs);
            n = n + 1
        elseif p == "Q" then
            local vs = parse_vs(params[n]);
            n = n + 1;
            rs = rs - 5
        ...
        elseif p == "C" then
            rs = rs - 5;
            op = op + shl(parse_cond(params[n]), rs);
            n = n + 1
        elseif p == "X" then
            rs = rs - 5;
            op = op + shl(parse_cr(params[n]), rs + 2);
            n = n + 1
        elseif p == "1" then
            rs = rs - 5;
        ...
        elseif p == "3" then
            rs = rs - 5;
            op = op + parse_imm(params[n], 3, rs, 0, false);
            n = n + 1
        elseif p == "P" then
            rs = rs - 5;
        ...
        elseif p == "Y" then
            rs = rs - 5;
            op = op + parse_imm(params[n], 1, rs + 4, 0, false);
            n = n + 1
        elseif p == "y" then
            rs = rs - 5;
            op = op + parse_imm(params[n], 1, rs + 3, 0, false);
            n = n + 1
        elseif p == "Z" then
            rs = rs - 5;
            op = op + parse_imm(params[n], 2, rs + 3, 0, false);
            n = n + 1
        elseif p == "z" then
            rs = rs - 5;
            op = op + parse_imm(params[n], 2, rs + 2, 0, false);
            n = n + 1
        elseif p == "W" then
        ...
        elseif p == "G" then
            ...
            n = n + 1
        elseif p == "H" then
            op = op + parse_shiftmask(params[n], true);
            n = n + 1
        elseif p == "M" then
            op = op + parse_shiftmask(params[n], false);
            n = n + 1
        elseif p == "J" or p == "K" then
            ...
            n = n + 1
        elseif p == "0" then
            if band(shr(op, rs), 31) == 0 then
                werror("cannot use r0")
            end
        elseif p == "=" or p == "%" then
        ...
        elseif p == ":" then
            if band(shr(op, rs), 1) ~= 0 then
                werror("register pair expected")
            end
        elseif p == "-" then
            rs = rs - 5
        elseif p == "." then
