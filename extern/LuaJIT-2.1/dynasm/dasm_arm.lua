...
local function parse_imm(imm, bits, shift, scale, signed)
    imm = match(imm, "^#(.*)$")
    if not imm then
        werror("expected immediate operand")
    end
...
local function parse_imm16(imm)
    ...
    if n then
        if shr(n, 16) == 0 then
            return band(n, 0x0fff) + shl(band(n, 0xf000), 4)
        end
        werror("out of range immediate `" .. imm .. "'")
    else
...
local function parse_load(params, nparams, n, op)
    ...
    if (ldrd or oplo == 240) then
        d = band(shr(op, 12), 15)
        if band(d, 1) ~= 0 then
            werror("odd destination register")
        end
    end
    local pn = params[n]
    ...
    if p2 then
        ...
        if imm then
            ...
            if ldrd and (m == d or m - 1 == d) then
                werror("register conflict")
            end
...
local function parse_template(params, template, nparams, pos)
    ...
    for p in gmatch(sub(template, 9), ".") do
        local q = params[n]
        if p == "D" then
            op = op + shl(parse_gpr(q), 12);
            n = n + 1
        elseif p == "N" then
            op = op + shl(parse_gpr(q), 16);
            n = n + 1
        elseif p == "S" then
            op = op + shl(parse_gpr(q), 8);
            n = n + 1
        elseif p == "M" then
            op = op + parse_gpr(q);
            n = n + 1
        elseif p == "d" then
            local r, h = parse_vr(q, vr);
            op = op + shl(r, 12) + shl(h, 22);
            n = n + 1
        elseif p == "n" then
            local r, h = parse_vr(q, vr);
            op = op + shl(r, 16) + shl(h, 7);
            n = n + 1
        elseif p == "m" then
            local r, h = parse_vr(q, vr);
            op = op + r + shl(h, 5);
        ...
        elseif p == "C" then
            -- blx gpr vs. blx label.
            if match(q, "^([%w_]+):(r1?[0-9])$") or match(q, "^r(1?[0-9])$") then
                op = op + parse_gpr(q)
            else
                if op < 0xe0000000 then
                    werror("unconditional instruction")
                end
                local mode, n, s = parse_label(q, false)
                waction("REL_" .. mode, n, s, 1)
                op = 0xfa000000
        ...
        elseif p == "R" then
            ...
            n = n + 1
        elseif p == "r" then
            op = op + parse_vrlist(q);
            n = n + 1
        elseif p == "W" then
            op = op + parse_imm16(q);
            n = n + 1
        elseif p == "v" then
            op = op + parse_imm(q, 5, 7, 0, false);
            n = n + 1
        elseif p == "w" then
            local imm = match(q, "^#(.*)$")
            if imm then
        ...
        elseif p == "T" then
            op = op + parse_imm(q, 24, 0, 0, false);
            n = n + 1
        elseif p == "s" then
            -- Ignored.
        else
