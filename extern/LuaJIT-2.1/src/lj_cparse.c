...
#define cp_str_is(str, k) ((str)->len == sizeof(k) - 1 && !memcmp(strdata(str), k, sizeof(k) - 1))

/* Check string against a linear list of matches. */
int
lj_cparse_case(GCstr *str, const char *match)
{
    MSize len;
    int   n;
...
static const char *
cp_tok2str(CPState *cp, CPToken tok)
{
...
static LJ_AINLINE CPChar
cp_rawpeek(CPState *cp)
{
...
static LJ_NOINLINE CPChar
cp_get_bs(CPState *cp)
{
...
LJ_NORET static void
cp_errmsg(CPState *cp, CPToken tok, ErrMsg em, ...)
{
...
LJ_NORET LJ_NOINLINE static void
cp_err_token(CPState *cp, CPToken tok)
{
    cp_errmsg(cp, cp->tok, LJ_ERR_XTOKEN, cp_tok2str(cp, tok));
}

LJ_NORET LJ_NOINLINE static void
cp_err_badidx(CPState *cp, CType *ct)
{
...
static void
cp_check(CPState *cp, CPToken tok)
{
    if (cp->tok != tok)
    {
        cp_err_token(cp, tok);
...
static void
cp_expr_comma(CPState *cp, CPValue *k)
{
    do
    {
        cp_expr_sub(cp, k, 0);
...
static void
cp_expr_sizeof(CPState *cp, CPValue *k, int wantsz)
{
    CTSize sz;
    CTInfo info;
    if (cp_opt(cp, '('))
...
static void
cp_expr_prefix(CPState *cp, CPValue *k)
{
...
static void
cp_expr_postfix(CPState *cp, CPValue *k)
{
    for (;;)
    {
        CType *ct;
...
static void
cp_expr_infix(CPState *cp, CPValue *k, int pri)
{
    CPValue k2;
    k2.u32 = 0;
...
static void
cp_expr_unary(CPState *cp, CPValue *k)
{
...
static void
cp_expr_sub(CPState *cp, CPValue *k, int pri)
{
    cp_expr_unary(cp, k);
...
static void
cp_expr_kint(CPState *cp, CPValue *k)
{
    CType *ct;
    cp_expr_sub(cp, k, 0);
...
static CPDeclIdx
cp_add(CPDecl *decl, CTInfo info, CTSize size)
{
...
static CPDeclIdx
cp_push(CPDecl *decl, CTInfo info, CTSize size)
{
...
static void
cp_push_attributes(CPDecl *decl)
{
    ...
    { /* Ok to modify in-place. */
        ...
        {
            ct->info
                = (ct->info & (CTMASK_NUM | CTF_VARARG | CTMASK_CID)) + (decl->fattr & ~CTMASK_CID);
        }
#endif
    }
    else
...
static void
cp_push_type(CPDecl *decl, CTypeID id)
{
...
static CTypeID
cp_decl_intern(CPState *cp, CPDecl *decl)
{
    CTypeID   id    = 0;
    CPDeclIdx idx   = 0;
                ...
                if (!(info & CTF_BOOL))
                {
                    ...
                    { /* Vector size set? */
                        ...
                        if (vsize >= esize)
                        {
                            ...
                            info
                                = CTINFO(CT_ARRAY, (info & CTF_QUAL) + CTF_VECTOR + CTALIGN(vsize));
                        }
                    }
                }
            }
...
static CTypeID
cp_decl_constinit(CPState *cp, CType **ctp, CTypeID ctypeid)
{
...
static void
cp_decl_align(CPState *cp, CPDecl *decl)
{
...
static void
cp_decl_asm(CPState *cp, CPDecl *decl)
{
    UNUSED(decl);
    cp_next(cp);
    cp_check(cp, '(');
...
static void
cp_decl_mode(CPState *cp, CPDecl *decl)
{
    cp_check(cp, '(');
    if (cp->tok == CTOK_IDENT)
    {
...
static void
cp_decl_gccattribute(CPState *cp, CPDecl *decl)
{
    cp_next(cp);
    cp_check(cp, '(');
    cp_check(cp, '(');
...
static void
cp_decl_msvcattribute(CPState *cp, CPDecl *decl)
{
    cp_next(cp);
    cp_check(cp, '(');
...
static void
cp_decl_attributes(CPState *cp, CPDecl *decl)
{
    for (;;)
    {
        switch (cp->tok)
...
static CTypeID
cp_struct_name(CPState *cp, CPDecl *sdecl, CTInfo info)
{
    CTypeID sid;
    CType  *ct;
...
static CTSize
cp_field_align(CPState *cp, CType *ct, CTInfo info)
{
...
static void
cp_struct_layout(CPState *cp, CTypeID sid, CTInfo sattr)
{
    ...
    while (fieldid)
    {
        ...
        if (ctype_isfield(ct->info) || (ctype_isxattrib(ct->info, CTA_SUBTYPE) && attr))
        {
            ...
            { /* Bitfield. */
                ...
                {
                    ct->info = CTINFO(CT_FIELD, ctype_cid(ct->info)) + CTALIGN(lj_fls(sz));
                    ct->size = (bofs >> 3); /* Store field offset. */
                }
                else
                {
...
static CTypeID
cp_decl_struct(CPState *cp, CPDecl *sdecl, CTInfo sinfo)
{
                ...
                else
                {
                    ...
                    { /* Regular field. */
                        ...
                        if (!decl.name)
                        {
                            ...
                            ct->size = ctype_isstruct(tct->info) ? (decl.attr | 0x80000000u)
                                                                 : 0; /* For layout phase. */
                            goto add_field;
                        }
                    }
                    ...
                    { /* Bitfield. */
                        bsz = decl.bits;
                        if (!ctype_isinteger_or_bool(tct->info) || (bsz == 0 && decl.name)
...
static CTypeID
cp_decl_enum(CPState *cp, CPDecl *sdecl)
{
...
static CPscl
cp_decl_spec(CPState *cp, CPDecl *decl, CPscl scl)
{
    uint32_t cds = 0, sz = 0;
    CTypeID  tdef = 0;
...
static void
cp_decl_array(CPState *cp, CPDecl *decl)
{
...
static void
cp_decl_func(CPState *cp, CPDecl *fdecl)
{
    CTSize  nargs  = 0;
        ...
        do
        {
            ...
            else if (ctype_isrefarray(ct->info))
            {
                ctypeid = lj_ctype_intern(
                    cp->cts, CTINFO(CT_PTR, CTALIGN_PTR | ctype_cid(ct->info)), CTSIZE_PTR);
            }
            else if (ctype_isfunc(ct->info))
            {
                ctypeid
                    = lj_ctype_intern(cp->cts, CTINFO(CT_PTR, CTALIGN_PTR | ctypeid), CTSIZE_PTR);
            }
            /* Add new parameter. */
...
/* Parse declarator. */
static void
cp_declarator(CPState *cp, CPDecl *decl)
{
    if (++cp->depth > CPARSE_MAX_DECLDEPTH)
    {
        cp_err(cp, LJ_ERR_XLEVELS);
...
/* Handle pragmas. */
static void
cp_pragma(CPState *cp, BCLine pragmaline)
{
    cp_next(cp);
...
/* Handle line number. */
static void
cp_line(CPState *cp, BCLine hashline)
{
...
static TValue *
cpcparser(lua_State *L, lua_CFunction dummy, void *ud)
{
