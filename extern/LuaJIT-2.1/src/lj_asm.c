...
#define opisfusableload(o)                                                                       \
    ((o) == IR_ALOAD || (o) == IR_HLOAD || (o) == IR_ULOAD || (o) == IR_FLOAD || (o) == IR_XLOAD \
     || (o) == IR_SLOAD || (o) == IR_VLOAD)

/* Sparse limit checks using a red zone before the actual limit. */
#define MCLIM_REDZONE 64

static LJ_NORET LJ_NOINLINE void
...
static LJ_AINLINE void
ra_setkref(ASMState *as, Reg r, intptr_t k)
{
...
static uint32_t
ra_random_bits(ASMState *as, uint32_t nbits)
{
    uint64_t b   = as->prngbits;
    uint32_t res = (1u << nbits) - 1u;
    if (b <= res)
    {
        b = lj_prng_u64(&as->prngstate) | (1ull << 63);
    }
    res &= (uint32_t)b;
    as->prngbits = b >> nbits;
    return res;
...
static Reg
rset_pickrandom(ASMState *as, RegSet rs)
{
...
static void
ra_dprintf(ASMState *as, const char *fmt, ...)
{
    char   *p;
    va_list argp;
    va_start(argp, fmt);
...
static Reg
ra_rematk(ASMState *as, IRRef ref)
{
    IRIns *ir;
    Reg    r;
    if (ra_iskref(ref))
    ...
    lj_assertA(!ra_hasspill(ir->s), "rematk of K%03d has spill slot [%x]", REF_BIAS - ref, ir->s);
    ra_free(as, r);
    ra_modified(as, r);
    ...
    {
        lj_assertA(ir->o == IR_KINT || ir->o == IR_KGC || ir->o == IR_KPTR || ir->o == IR_KKPTR
                       || ir->o == IR_KNULL,
                   "rematk of bad IR op %d",
                   ir->o);
...
static int32_t
ra_spill(ASMState *as, IRIns *ir)
{
    int32_t slot = ir->s;
    lj_assertA(ir >= as->ir + REF_TRUE, "spill of K%03d", REF_BIAS - (int)(ir - as->ir));
    if (!ra_hasspill(slot))
    {
        if (irt_is64(ir->t))
        {
...
static Reg
ra_releasetmp(ASMState *as, IRRef ref)
{
    IRIns *ir = IR(ref);
    Reg    r  = ir->r;
    lj_assertA(ra_hasreg(r), "release of TMP%d has no reg", ref - ASMREF_TMP1 + 1);
    lj_assertA(
        !ra_hasspill(ir->s), "release of TMP%d has spill slot [%x]", ref - ASMREF_TMP1 + 1, ir->s);
    ra_free(as, r);
    ra_modified(as, r);
    ir->r = RID_INIT;
    return r;
...
static Reg
ra_restore(ASMState *as, IRRef ref)
{
...
static void
ra_save(ASMState *as, IRIns *ir, Reg r)
{
...
static Reg
ra_pick(ASMState *as, RegSet allow)
{
...
static Reg
ra_scratch(ASMState *as, RegSet allow)
{
...
static void
ra_evictset(ASMState *as, RegSet drop)
{
    RegSet work;
    as->modset |= drop;
#if !LJ_SOFTFP
...
static Reg
ra_allock(ASMState *as, intptr_t k, RegSet allow)
{
        ...
        if (ref < ASMREF_L)
        {
            ...
            else
            {
                ...
                if ((ir->o == IR_KINT64 && k == (int64_t)ir_kint64(ir)->u64) ||
                    ...
                    || ((ir->o == IR_KPTR || ir->o == IR_KKPTR) && k == (intptr_t)ir_kptr(ir))
#else
...
static void
ra_allockreg(ASMState *as, intptr_t k, Reg r)
{
...
*/
static Reg
ra_allocref(ASMState *as, IRRef ref, RegSet allow)
{
    IRIns *ir   = IR(ref);
    RegSet pick = as->freeset & allow;
    Reg    r;
    lj_assertA(ra_noreg(ir->r), "IR %04d already has reg %d", ref - REF_BIAS, ir->r);
    if (pick)
    {
...
static Reg
ra_alloc1(ASMState *as, IRRef ref, RegSet allow)
{
    Reg r = IR(ref)->r;
...
static void
ra_addrename(ASMState *as, Reg down, IRRef ref, SnapNo snapno)
{
    IRRef ren;
...
static void
ra_rename(ASMState *as, Reg down, Reg up)
{
    ...
    as->cost[down] = 0;
    lj_assertA(
        (down < RID_MAX_GPR) == (up < RID_MAX_GPR), "rename between GPR/FPR %d and %d", down, up);
...
*/
static Reg
ra_dest(ASMState *as, IRIns *ir, RegSet allow)
{
    Reg dest = ir->r;
    if (ra_hasreg(dest))
    {
...
static void
ra_destreg(ASMState *as, IRIns *ir, Reg r)
{
...
*/
static void
ra_left(ASMState *as, Reg dest, IRRef lref)
{
    IRIns *ir   = IR(lref);
    Reg    left = ir->r;
    if (ra_noreg(left))
    {
        if (irref_isk(lref))
        {
            ...
            {
                lj_assertA(ir->o == IR_KINT || ir->o == IR_KGC || ir->o == IR_KPTR
                               || ir->o == IR_KKPTR || ir->o == IR_KNULL,
                           "K%03d has bad IR op %d",
                           REF_BIAS - lref,
                           ir->o);
...
static void
ra_leftov(ASMState *as, Reg dest, IRRef lref)
{
    IRIns *ir   = IR(lref);
    Reg    left = ir->r;
    if (ra_noreg(left))
...
static void
ra_destpair(ASMState *as, IRIns *ir)
{
    ...
    else if (desthi == RID_RETLO)
    {
        ...
        if (destlo != RID_RETLO)
        {
            emit_movrr(as, irx, destlo, RID_RETLO);
        }
    }
    else
    {
...
static int
asm_sunk_store(ASMState *as, IRIns *ira, IRIns *irs)
{
    if (irs->s == 255)
    {
...
static void
asm_snap_alloc1(ASMState *as, IRRef ref)
{
    IRIns *ir = IR(ref);
    if (!irref_isk(ref))
    {
        ...
        if (ir->r == RID_SINK || ir->r == RID_SUNK)
        {
            ...
            { /* Allocate stored values for TNEW, TDUP and CNEW. */
                ...
                for (irs = IR(as->snapref - 1); irs > ir; irs--)
                {
                    ...
                    {
                        lj_assertA(irs->o == IR_ASTORE || irs->o == IR_HSTORE || irs->o == IR_FSTORE
                                       || irs->o == IR_XSTORE,
                                   "sunk store IR %04d has bad op %d",
                                   (int)(irs - as->ir) - REF_BIAS,
                                   irs->o);
...
static void
asm_snap_alloc(ASMState *as, int snapno)
{
...
*/
static int
asm_snap_checkrename(ASMState *as, IRRef ren)
{
    if (bloomtest(as->snapfilt1, ren) && bloomtest(as->snapfilt2, hashrot(ren, ren + HASH_BIAS)))
    {
        IRIns *ir = IR(ren);
...
/* Explicit GC step. */
static void
asm_gcstep(ASMState *as, IRIns *ir)
{
    IRIns *ira;
...
static void
asm_bufhdr(ASMState *as, IRIns *ir)
{
...
#endif
        default:
            lj_assertA(0, "bad BUFHDR op2 %d", ir->op2);
            break;
    }
#if LJ_TARGET_X86ORX64
...
static void
asm_bufput(ASMState *as, IRIns *ir)
{
    ...
    args[1]                = ir->op2; /* GCstr * */
    irs                    = IR(ir->op2);
    lj_assertA(irt_isstr(irs->t), "BUFPUT of non-string IR %04d", ir->op2 - REF_BIAS);
    if (irs->o == IR_KGC)
    {
        GCstr *s = ir_kstr(irs);
        if (s->len == 1)
        {                                    /* Optimize put of single-char string constant. */
    ...
    else if (mayfuse(as, ir->op2) && ra_noreg(irs->r))
    {
        ...
        { /* Fuse number to string conversions. */
            ...
            {
                lj_assertA(
                    irt_isinteger(IR(irs->op1)->t), "TOSTR of non-numeric IR %04d", irs->op1);
                args[1] = irs->op1; /* int */
...
static void
asm_bufstr(ASMState *as, IRIns *ir)
{
...
static void
asm_tostr(ASMState *as, IRIns *ir)
{
    const CCallInfo *ci;
    IRRef            args[2];
    asm_snap_prep(as);
...
static void
asm_conv64(ASMState *as, IRIns *ir)
{
    ...
    lj_assertA((ir - 1)->o == IR_CONV && ir->o == IR_HIOP,
               "not a CONV/HIOP pair at IR %04d",
...
static void
asm_newref(ASMState *as, IRIns *ir)
{
...
static void
asm_tmpref(ASMState *as, IRIns *ir)
{
...
static void
asm_collectargs(ASMState *as, IRIns *ir, const CCallInfo *ci, IRRef *args)
{
...
static uint32_t
asm_callx_flags(ASMState *as, IRIns *ir)
{
    uint32_t nargs = 0;
    if (ir->op1 != REF_NIL)
...
static void
asm_callid(ASMState *as, IRIns *ir, IRCallID id)
{
...
static void
asm_phi_break(ASMState *as, RegSet blocked, RegSet blockedby, RegSet allow)
{
...
static void
asm_fuseequal(ASMState *as, IRIns *ir)
{
    /* Fuse HREF + EQ/NE. */
...
static void
asm_alen(ASMState *as, IRIns *ir)
{
    asm_callid(as, ir, ir->op2 == REF_NIL ? IRCALL_lj_tab_len : IRCALL_lj_tab_len_hint);
}
...
asm_ir(ASMState *as, IRIns *ir)
{
    switch ((IROp)ir->o)
    {
        ...
        case IR_NOP:
        case IR_XBAR:
            lj_assertA(!ra_used(ir), "IR %04d not unused", (int)(ir - as->ir) - REF_BIAS);
            break;
        case IR_USE:
        ...
        case IR_TOBIT:
            ...
            lj_assertA(0, "IR %04d with unused op %d", (int)(ir - as->ir) - REF_BIAS, ir->o);
            break;
#else
        case IR_DIV:
            asm_div(as, ir);
...
asm_head_side(ASMState *as)
{
    ...
    for (i = as->stopins; i > REF_BASE; i--)
    {
        ...
        {
            rset_clear(pallow, regsp_reg(rs));
        }
    }
...
static BCReg
asm_baseslot(ASMState *as, SnapShot *snap, int *gotframe)
{
    SnapEntry *map = &as->T->snapmap[snap->mapofs];
    MSize      n;
    ...
    for (lastir = IR(nins); ir < lastir; ir++)
    {
        ...
        switch (ir->o)
        {
            ...
            case IR_CALLS: {
                ...
                if (inloop)
                {
                    as->modset |= (ci->flags & CCI_NOFPRCLOBBER) ? (RSET_SCRATCH & ~RSET_FPR)
                                                                 : RSET_SCRATCH;
                }
                continue;
            }
...
/* Assemble a trace. */
void
lj_asm_trace(jit_State *J, GCtrace *T)
{
    ASMState  as_;
    ASMState *as = &as_;
    ...
    */
    for (;;)
    {
        ...
        { /* IR didn't grow? */
            ...
            memcpy(J->curfinal->ir + as->orignins,
                   T->ir + as->orignins,
