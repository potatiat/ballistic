...
static void
rec_check_ir(jit_State *J)
{
    IRRef i, nins = J->cur.nins, nk = J->cur.nk;
    lj_assertJ(nk <= REF_BIAS && nins >= REF_BIAS && nins < 65536, "inconsistent IR layout");
    for (i = nk; i < nins; i++)
    {
        IRIns      *ir   = IR(i);
...
rec_check_slots(jit_State *J)
{
    ...
    for (s = 0; s < nslots; s++)
    {
        ...
        if (tr)
        {
...
#if LJ_FR2
                lj_assertJ(
                    !ref || ir_knum(ir)->u64 == tv->u64, "cont slot %d continuation mismatch", s);
#else
                lj_assertJ(
                    ir_kptr(ir) == gcrefp(tv->gcr, void), "cont slot %d continuation mismatch", s);
#endif
                lj_assertJ((J->slot[s + 1 + LJ_FR2] & TREF_FRAME),
                           "cont slot %d not followed by frame",
                           s);
                depth++;
            }
            ...
            else
            {
                ...
                lj_assertJ(tvisnumber(tv) ? tref_isnumber(tr) : itype2irt(tv) == tref_type(tr),
                ...
                { /* Compare constants. */
                    ...
                    lj_assertJ((tvisnum(&tvk) && tvisnan(&tvk)) ? (tvisnum(tv) && tvisnan(tv))
                                                                : lj_obj_equal(tv, &tvk),
    ...
    lj_assertJ(J->framedepth == depth, "frame depth mismatch %d vs %d", J->framedepth, depth);
}
#endif
...
static TRef
sloadt(jit_State *J, int32_t slot, IRType t, int mode)
{
...
static TRef
sload(jit_State *J, int32_t slot)
{
...
*/
int
lj_record_objcmp(jit_State *J, TRef a, TRef b, cTValue *av, cTValue *bv)
{
...
TRef
lj_record_constify(jit_State *J, cTValue *o)
{
    if (tvisgcv(o))
    {
...
TRef
lj_record_vload(jit_State *J, TRef ref, MSize idx, IRType t)
{
...
static void
canonicalize_slots(jit_State *J)
{
    BCReg s;
    if (LJ_DUALNUM)
    {
...
/* Stop recording. */
void
lj_record_stop(jit_State *J, TraceLink linktype, TraceNo lnk)
{
...
static TRef
find_kinit(jit_State *J, const BCIns *endpc, BCReg slot, IRType t)
{
...
static TRef
fori_load(jit_State *J, BCReg slot, IRType t, int mode)
{
    int conv = (tvisint(&J->L->base[slot]) != (t == IRT_INT)) ? IRSLOAD_CONVERT : 0;
    return sloadt(J,
                  (int32_t)slot,
                  t
                      + (((mode & IRSLOAD_TYPECHECK) || (conv && t == IRT_INT && !(mode >> 16)))
                             ? IRT_GUARD
                             : 0),
                  mode + conv);
}
...
static TRef
fori_conv(jit_State *J, TRef tr, IRType t)
{
    if (t == IRT_INT)
    {
...
static TRef
fori_arg(jit_State *J, const BCIns *fori, BCReg slot, IRType t, int mode)
{
    TRef tr = J->base[slot];
    if (tr)
    {
...
static LoopEvent
rec_for_iter(IROp *op, cTValue *o, int isforl)
{
...
static void
rec_for_check(jit_State *J, IRType t, int dir, TRef stop, TRef step, int init)
{
    if (!tref_isk(step))
    {
...
static void
rec_for_loop(jit_State *J, const BCIns *fori, ScEvEntry *scev, int init)
{
    BCReg    ra  = bc_a(*fori);
    ...
    int      mode
        = IRSLOAD_INHERIT + ((!LJ_DUALNUM || tvisint(tv) == (t == IRT_INT)) ? IRSLOAD_READONLY : 0);
    TRef stop = fori_arg(J, fori, ra + FORL_STOP, t, mode);
    TRef step = fori_arg(J, fori, ra + FORL_STEP, t, mode);
    int  tc, dir = rec_for_direction(&tv[FORL_STEP]);
    lj_assertJ(bc_op(*fori) == BC_FORI || bc_op(*fori) == BC_JFORI,
               "bad bytecode %d instead of FORI/JFORI",
               bc_op(*fori));
    scev->t.irt = t;
    scev->dir   = dir;
...
static LoopEvent
rec_for(jit_State *J, const BCIns *fori, int isforl)
{
    BCReg     ra = bc_a(*fori);
    ...
    if ((tvisnum(&tv[FORL_IDX]) && tvisnan(&tv[FORL_IDX]))
        ...
        || (tvisnum(&tv[FORL_STEP]) && tvisnan(&tv[FORL_STEP])) || tvismzero(&tv[FORL_STEP]))
    {
...
/* Record ITERL/JITERL. */
static LoopEvent
rec_iterl(jit_State *J, const BCIns iterins)
{
...
static LoopEvent
rec_loop(jit_State *J, BCReg ra, int skip)
{
    if (ra < J->maxslot)
    {
        J->maxslot = ra;
...
static int
innerloopleft(jit_State *J, const BCIns *pc)
{
    ptrdiff_t i;
    for (i = 0; i < PENALTY_SLOTS; i++)
    {
        ...
        {
            if ((J->penalty[i].reason == LJ_TRERR_LLEAVE || J->penalty[i].reason == LJ_TRERR_LINNER)
...
static void
rec_loop_interp(jit_State *J, const BCIns *pc, LoopEvent ev)
{
    if (J->parent == 0 && J->exitno == 0)
    {
        if (pc == J->startpc && J->framedepth + J->retdepth == 0)
        {
            if (bc_op(J->cur.startins) == BC_ITERN)
            {
        ...
        { /* Entering inner loop? */
            ...
            */
            ...
            if ((ev != LOOPEV_ENTERLO && J->loopref && J->cur.nins - J->loopref > 24)
                || --J->loopunroll < 0)
            {
...
static void
rec_loop_jit(jit_State *J, TraceNo lnk, LoopEvent ev)
{
    if (J->parent == 0 && J->exitno == 0)
...
/* Record ITERN. */
static LoopEvent
rec_itern(jit_State *J, BCReg ra, BCReg rb)
{
#if LJ_BE
    ...
    */
    ...
    if (J->pc == J->startpc && J->framedepth + J->retdepth == 0 && J->parent == 0 && J->exitno == 0)
    {
...
/* Record ISNEXT. */
static void
rec_isnext(jit_State *J, BCReg ra)
{
    cTValue *b = &J->L->base[ra - 3];
    if (tvisfunc(b) && funcV(b)->c.ffid == FF_next && tvistab(b + 1) && tvisnil(b + 2))
    {
    ...
    { /* Abort trace. Interpreter will despecialize bytecode. */
...
static int
rec_profile_need(jit_State *J, GCproto *pt, const BCIns *pc)
{
    GCproto *ppt;
    lj_assertJ(J->prof_mode == 'f' || J->prof_mode == 'l', "bad profiler mode %c", J->prof_mode);
    if (!pt)
    {
        return 0;
    }
...
static void
rec_profile_ins(jit_State *J, const BCIns *pc)
{
...
static TRef
rec_call_specialize(jit_State *J, GCfunc *fn, TRef tr)
{
    TRef kfunc;
    if (isluafunc(fn))
    {
...
/* Record call setup. */
static void
rec_call_setup(jit_State *J, BCReg func, ptrdiff_t nargs)
{
    RecordIndex ix;
...
/* Record call. */
void
lj_record_call(jit_State *J, BCReg func, ptrdiff_t nargs)
{
...
/* Record tail call. */
void
lj_record_tailcall(jit_State *J, BCReg func, ptrdiff_t nargs)
{
    rec_call_setup(J, func, nargs);
...
static int
check_downrec_unroll(jit_State *J, GCproto *pt)
{
    IRRef ptref;
...
/* Record return. */
void
lj_record_ret(jit_State *J, BCReg rbase, ptrdiff_t gotresults)
{
    ...
    if (J->framedepth == 0 && J->pt && bc_isret(bc_op(*J->pc))
        && (!frame_islua(frame)
            || (J->parent == 0 && J->exitno == 0 && !bc_isret(bc_op(J->cur.startins)))))
    {
    ...
    { /* Return to Lua frame. */
        ...
        else if (1 + pt->framesize >= LJ_MAX_JSLOTS || J->baseslot + J->maxslot >= LJ_MAX_JSLOTS)
        {
            lj_trace_err(J, LJ_TRERR_STACKOV);
        }
        else
    ...
    { /* Return to continuation frame. */
        ...
        else
        {
            ...
            lj_assertJ(cont == lj_cont_condf || cont == lj_cont_condt, "bad continuation type");
        }
    }
    else
    {
...
static BCReg
rec_mm_prep(jit_State *J, ASMFunction cont)
{
...
int
lj_record_mm_lookup(jit_State *J, RecordIndex *ix, MMS mm)
{
    RecordIndex mix;
    GCtab      *mt;
    ...
    else
    {
        ...
        ix->mt = mix.tab
            = lj_ir_ggfload(J, IRT_TAB, GG_OFS(g.gcroot[GCROOT_BASEMT + itypemap(&ix->tabv)]));
        goto nocheck;
    }
...
static TRef
rec_mm_arith(jit_State *J, RecordIndex *ix, MMS mm)
{
...
static TRef
rec_mm_len(jit_State *J, TRef tr, TValue *tv)
{
    RecordIndex ix;
    ix.tab = tr;
...
static void
rec_mm_callcomp(jit_State *J, RecordIndex *ix, int op)
{
...
static void
rec_mm_equal(jit_State *J, RecordIndex *ix, int op)
{
    ix->tab = ix->val;
...
static void
rec_mm_comp(jit_State *J, RecordIndex *ix, int op)
{
    ix->tab = ix->val;
    copyTV(J->L, &ix->tabv, &ix->valv);
    while (1)
...
static void
rec_mm_comp_cdata(jit_State *J, RecordIndex *ix, int op, MMS mm)
{
    lj_snap_add(J);
...
static void
rec_idx_bump(jit_State *J, RecordIndex *ix)
{
...
/* Record bounds-check. */
static void
rec_idx_abc(jit_State *J, TRef asizeref, TRef ikey, uint32_t asize)
{
    ...
    if ((J->flags & (JIT_F_OPT_LOOP | JIT_F_OPT_ABC)) == (JIT_F_OPT_LOOP | JIT_F_OPT_ABC))
    {
        ...
        {
            int32_t stop;
            lj_assertJ(irt_isint(J->scev.t) && ir->o == IR_SLOAD, "only int SCEV supported");
...
static TRef
rec_idx_key(jit_State *J, RecordIndex *ix, IRRef *rbref, IRType1 *rbguard)
{
    TRef   key;
    ...
    *rbref       = 0;
    ...
    if (tref_isnumber(key))
    {
        ...
        {
            int64_t i64;
            if (!lj_num2int_check(numV(&ix->keyv), i64, k))
            {
                k = LJ_MAX_ASIZE;
            }
        }
    ...
    if (tref_isk(key))
    {
        ...
        if (hslot <= t->hmask * (GCSize)sizeof(Node) && hslot <= 65535 * (GCSize)sizeof(Node))
        {
            TRef node, kslot, hm;
...
TRef
lj_record_idx(jit_State *J, RecordIndex *ix)
{
    TRef     xref;
    IROp     xrefop, loadop;
    IRRef    rbref;
    ...
    handlemm:
        ...
        if (ix->mt == TREF_NIL && !ix->val && tref_isudata(ix->tab)
            && udataV(&ix->tabv)->udtype == UDTYPE_BUFFER && tref_istab(ix->mobj)
            && tref_isstr(ix->key) && tref_isk(ix->key))
        {
            cTValue *val = lj_tab_getstr(tabV(&ix->mobjv), strV(&ix->keyv));
            TRef     tr  = lj_record_constify(J, val);
            if (tr)
            {
                return tr; /* Specialize to the value, i.e. a method. */
            }
        }
#endif
...
static IRType
rec_next_types(GCtab *t, uint32_t idx)
{
    for (; idx < t->asize; idx++)
    {
...
int
lj_record_next(jit_State *J, RecordIndex *ix)
{
    IRType t, tkey, tval;
    TRef   trvk;
    ...
    if (ix->mobj || tkey == IRT_NIL)
    {
        ...
        if (!ix->mobj)
        {
            emitir(IRTGI(IR_NE), idx, lj_ir_kint(J, -1));
        }
        ix->mobj = idx;
    }
...
static void
rec_tsetm(jit_State *J, BCReg ra, BCReg rn, int32_t i)
{
    RecordIndex ix;
...
static int
rec_upvalue_constify(jit_State *J, GCupval *uvp)
{
    if (uvp->immutable)
    {
        cTValue *o = uvval(uvp);
...
static TRef
rec_upvalue(jit_State *J, uint32_t uv, TRef val)
{
        ...
        if (uvval(uvp) >= tvref(J->L->stack) && uvval(uvp) < tvref(J->L->maxstack))
        {
            ...
            { /* Aliases an SSA slot? */
                ...
                emitir(
                    IRTG(IR_EQ, IRT_PGC),
                    REF_BASE,
                    emitir(IRT(IR_ADD, IRT_PGC), uref, lj_ir_kintpgc(J, (slot - 1 - LJ_FR2) * -8)));
...
static void
check_call_unroll(jit_State *J, TraceNo lnk)
{
...
static void
rec_func_jit(jit_State *J, TraceNo lnk)
{
    GCtrace *T;
    rec_func_setup(J);
    T = traceref(J, lnk);
...
static void
rec_varg(jit_State *J, BCReg dst, ptrdiff_t nresults)
{
    ...
    { /* Unknown number of varargs passed to trace. */
        ...
        { /* Known fixed number of results. */
            ...
            if (nvararg > 0)
            {
                ...
                vbase = emitir(
                    IRT(IR_ADD, IRT_PGC), vbase, lj_ir_kintpgc(J, frofs - 8 * (1 + LJ_FR2)));
                for (i = 0; i < nload; i++)
                {
        ...
        { /* y = select(x, ...) */
            ...
            if (idx != 0 && idx <= nvararg)
            {
                ...
                vbase
                    = emitir(IRT(IR_ADD, IRT_PGC), vbase, lj_ir_kintpgc(J, frofs - (8 << LJ_FR2)));
...
static TRef
rec_tnew(jit_State *J, uint32_t ah)
{
...
static TValue *
rec_mm_concat_cp(lua_State *L, lua_CFunction dummy, void *ud)
{
    ...
    if (tref_isnumber_str(top[0]) && tref_isnumber_str(top[-1]))
    {
        ...
        for (trp = top; trp >= base; trp--)
        {
            ...
            {
                *trp = emitir(
                    IRT(IR_TOSTR, IRT_STR), *trp, tref_isnum(*trp) ? IRTOSTR_NUM : IRTOSTR_INT);
            }
...
static TRef
rec_cat(jit_State *J, BCReg baseslot, BCReg topslot)
{
    lua_State   *L     = J->L;
...
/* Fixup comparison. */
static void
rec_comp_fixup(jit_State *J, const BCIns *pc, int cond)
{
    BCIns        jmpins = pc[1];
    ...
    switch (op)
    {
        ...
        case BC_TDUP:
            rc = emitir(IRTG(IR_TDUP, IRT_TAB),
                        lj_ir_ktab(J, gco2tab(proto_kgc(J->pt, ~(ptrdiff_t)rc))),
                        0);
#ifdef LUAJIT_ENABLE_TABLE_BUMP
