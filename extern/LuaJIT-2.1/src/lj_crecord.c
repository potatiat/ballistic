...
static GCcdata *
argv2cdata(jit_State *J, TRef tr, cTValue *o)
{
    GCcdata *cd;
    TRef     trtypeid;
    if (!tref_iscdata(tr))
...
static CTypeID
crec_constructor(jit_State *J, GCcdata *cd, TRef tr)
{
    CTypeID id;
    lj_assertJ(tref_iscdata(tr) && cd->ctypeid == CTID_CTYPEID, "expected CTypeID cdata");
...
static CTypeID
argv2ctype(jit_State *J, TRef tr, cTValue *o)
{
    if (tref_isstr(tr))
    {
        GCstr  *s = strV(o);
...
static IRType
crec_ct2irt(CTState *cts, CType *ct)
{
...
static MSize
crec_copy_struct(CRecMemList *ml, CTState *cts, CType *ct)
{
    CTypeID fid = ct->sib;
    MSize   mlp = 0;
    while (fid)
...
static MSize
crec_copy_unroll(CRecMemList *ml, CTSize len, CTSize step, IRType tp)
{
    CTSize ofs = 0;
    MSize  mlp = 0;
    if (tp == IRT_CDATA)
...
*/
static void
crec_copy_emit(jit_State *J, CRecMemList *ml, MSize mlp, TRef trdst, TRef trsrc)
{
    MSize i, j, rwin = 0;
    for (i = 0, j = 0; i < mlp;)
    {
...
static void
crec_copy(jit_State *J, TRef trdst, TRef trsrc, TRef trlen, CType *ct)
{
    if (tref_isk(trlen))
...
static MSize
crec_fill_unroll(CRecMemList *ml, CTSize len, CTSize step)
{
    CTSize ofs = 0;
    MSize  mlp = 0;
...
*/
static void
crec_fill_emit(jit_State *J, CRecMemList *ml, MSize mlp, TRef trdst, TRef trfill)
{
    MSize i;
    for (i = 0; i < mlp; i++)
    {
...
static void
crec_fill(jit_State *J, TRef trdst, TRef trlen, TRef trfill, CTSize step)
{
    if (tref_isk(trlen))
    { /* Length must be constant. */
        ...
        { /* Scatter U8 to U16/U32/U64. */
            if (CTSIZE_PTR == 8 && ml[0].tp == IRT_U64)
            {
                ...
                trfill = emitir(
                    IRT(IR_MUL, IRT_U64), trfill, lj_ir_kint64(J, U64x(01010101, 01010101)));
            }
            else
            {
                trfill = emitir(
                    IRTI(IR_MUL), trfill, lj_ir_kint(J, ml[0].tp == IRT_U16 ? 0x0101 : 0x01010101));
            }
        }
...
static TRef
crec_ct_ct(jit_State *J, CType *d, CType *s, TRef dp, TRef sp, void *svisnz)
{
    ...
    */
    ...
    switch (cconv_idx2(dinfo, sinfo))
    {
        ...
        conv_I_I:
            ...
            {
                sp = emitconv(
                    sp, dt, ssize < 4 ? IRT_INT : st, (sinfo & CTF_UNSIGNED) ? 0 : IRCONV_SEXT);
            }
...
#if LJ_SOFTFP || LJ_32
            if (st == IRT_FLOAT)
            { /* Uncommon. Simplify split backends. */
...
static TRef
crec_tv_ct(jit_State *J, CType *s, CTypeID sid, TRef sp)
{
    CTState *cts   = ctype_ctsG(J2G(J));
...
static TRef
crec_ct_tv(jit_State *J, CType *d, TRef dp, TRef sp, cTValue *sval)
{
    CTState *cts    = ctype_ctsG(J2G(J));
    ...
    else if (tref_isudata(sp))
    {
        ...
        if (ud->udtype == UDTYPE_IO_FILE || ud->udtype == UDTYPE_BUFFER)
        {
            ...
            sp = emitir(IRT(IR_FLOAD, IRT_PTR),
                        sp,
                        ud->udtype == UDTYPE_IO_FILE ? IRFL_UDATA_FILE : IRFL_SBUF_R);
        }
        else
        {
...
*/
static TRef
crec_reassoc_ofs(jit_State *J, TRef tr, ptrdiff_t *ofsp, MSize sz)
{
...
static void
crec_tailcall(jit_State *J, RecordFFData *rd, cTValue *tv)
{
...
static void
crec_index_meta(jit_State *J, CTState *cts, CType *ct, RecordFFData *rd)
{
...
static void
crec_index_bf(jit_State *J, RecordFFData *rd, TRef ptr, CTInfo info)
{
...
void LJ_FASTCALL
recff_cdata_index(jit_State *J, RecordFFData *rd)
{
...
static void
crec_finalizer(jit_State *J, TRef trcd, TRef trfin, cTValue *fin)
{
    if (tvisgcv(fin))
    {
        if (!trfin)
...
static void
crec_alloc(jit_State *J, RecordFFData *rd, CTypeID id)
{
    CTState *cts = ctype_ctsG(J2G(J));
    CTSize   sz;
    ...
    else
    {
        ...
        else if (ctype_isstruct(d->info))
        {
            ...
            MSize   i = 1;
            if (!J->base[1])
            { /* Handle zero-fill of struct-of-NYI. */
                fid = d->sib;
                while (fid)
                {
                    CType *df = ctype_get(cts, fid);
            ...
            while (fid)
            {
                ...
                if (ctype_isfield(df->info))
                {
                    ...
                    dp = emitir(
                        IRT(IR_ADD, IRT_PTR), trcd, lj_ir_kintp(J, df->size + sizeof(GCcdata)));
                    crec_ct_tv(J, dc, dp, sp, sval);
                    if ((d->info & CTF_UNION))
                    {
...
*/
static TRef
crec_call_args(jit_State *J, RecordFFData *rd, CTState *cts, CType *ct)
{
...
static int
crec_call(jit_State *J, RecordFFData *rd, GCcdata *cd)
{
    CTState *cts = ctype_ctsG(J2G(J));
    CType   *ct  = ctype_raw(cts, cd->ctypeid);
    CTInfo   info;
    ...
    if (ctype_isfunc(info))
    {
        ...
        tv.u64 = ((uintptr_t)cdata_getptr(cdataptr(cd), (LJ_64 && tp == IRT_P64) ? 8 : 4) >> 2)
                 | U64x(800000000, 00000000);
        ...
        else if (!(ctype_isnum(ctr_info) || ctype_isptr(ctr_info) || ctype_isenum(ctr_info))
                 || t == IRT_CDATA)
        {
            lj_trace_err(J, LJ_TRERR_NYICALL);
        }
        ...
        else if (t == IRT_PTR || (LJ_64 && t == IRT_P32) || t == IRT_I64 || t == IRT_U64
...
void LJ_FASTCALL
recff_cdata_call(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
...
static TRef
crec_arith_int64(jit_State *J, TRef *sp, CType **s, MMS mm)
{
    if (sp[0] && sp[1] && ctype_isnum(s[0]->info) && ctype_isnum(s[1]->info))
    {
        ...
        else
        {
            dt = IRT_I64;
            id = CTID_INT64;
            if (mm < MM_add && !((s[0]->info | s[1]->info) & CTF_FP) && s[0]->size == 4
                && s[1]->size == 4)
...
static TRef
crec_arith_ptr(jit_State *J, TRef *sp, CType **s, MMS mm)
{
    CTState *cts = ctype_ctsG(J2G(J));
    CType   *ctp = s[0];
    if (!(sp[0] && sp[1]))
...
static TRef
crec_arith_meta(jit_State *J, TRef *sp, CType **s, CTState *cts, RecordFFData *rd)
{
    cTValue *tv = NULL;
    if (J->base[0])
    {
...
void LJ_FASTCALL
recff_cdata_arith(jit_State *J, RecordFFData *rd)
{
    ...
    }
    {
        ...
        if ((mm == MM_len || mm == MM_concat
             || (!(tr = crec_arith_int64(J, sp, s, mm)) && !(tr = crec_arith_ptr(J, sp, s, mm))))
...
void LJ_FASTCALL
recff_clib_index(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
...
static TRef
crec_toint(jit_State *J, CTState *cts, TRef sp, TValue *sval)
{
    return crec_ct_tv(J, ctype_get(cts, CTID_INT32), 0, sp, sval);
}

void LJ_FASTCALL
recff_ffi_new(jit_State *J, RecordFFData *rd)
{
    crec_alloc(J, rd, argv2ctype(J, J->base[0], &rd->argv[0]));
}

void LJ_FASTCALL
recff_ffi_errno(jit_State *J, RecordFFData *rd)
{
    UNUSED(rd);
    if (J->base[0])
    {
...
void LJ_FASTCALL
recff_ffi_string(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
    TRef     tr  = J->base[0];
    if (tr)
...
void LJ_FASTCALL
recff_ffi_copy(jit_State *J, RecordFFData *rd)
{
    CTState *cts   = ctype_ctsG(J2G(J));
...
void LJ_FASTCALL
recff_ffi_fill(jit_State *J, RecordFFData *rd)
{
    CTState *cts   = ctype_ctsG(J2G(J));
...
void LJ_FASTCALL
recff_ffi_typeof(jit_State *J, RecordFFData *rd)
{
...
void LJ_FASTCALL
recff_ffi_istype(jit_State *J, RecordFFData *rd)
{
...
void LJ_FASTCALL
recff_ffi_abi(jit_State *J, RecordFFData *rd)
{
...
void LJ_FASTCALL
recff_ffi_xof(jit_State *J, RecordFFData *rd)
{
...
void LJ_FASTCALL
recff_ffi_gc(jit_State *J, RecordFFData *rd)
{
...
static CTypeID
crec_bit64_type(CTState *cts, cTValue *tv)
{
    if (tviscdata(tv))
    {
...
static TRef
crec_bit64_arg(jit_State *J, CType *d, TRef sp, TValue *sval)
{
...
void LJ_FASTCALL
recff_bit64_tobit(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
    TRef     tr  = crec_bit64_arg(J, ctype_get(cts, CTID_INT64), J->base[0], &rd->argv[0]);
...
int LJ_FASTCALL
recff_bit64_unary(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
...
int LJ_FASTCALL
recff_bit64_nary(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
    CTypeID  id  = 0;
    MSize    i;
...
int LJ_FASTCALL
recff_bit64_shift(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
    CTypeID  id;
    TRef     tsh = 0;
    ...
    {
        tsh = crec_bit64_arg(J, ctype_get(cts, CTID_INT64), J->base[1], &rd->argv[1]);
...
TRef
recff_bit64_tohex(jit_State *J, RecordFFData *rd, TRef hdr)
{
    CTState *cts = ctype_ctsG(J2G(J));
    CTypeID  id  = crec_bit64_type(cts, &rd->argv[0]);
...
void LJ_FASTCALL
lj_crecord_tonumber(jit_State *J, RecordFFData *rd)
{
    CTState *cts = ctype_ctsG(J2G(J));
...
TRef
lj_crecord_loadiu64(jit_State *J, TRef tr, cTValue *o)
{
    ...
    lj_needsplit(J);
    return emitir(IRT(IR_FLOAD, id == CTID_INT64 ? IRT_I64 : IRT_U64), tr, IRFL_CDATA_INT64);
}

#if LJ_HASBUFFER
TRef
lj_crecord_topcvoid(jit_State *J, TRef tr, cTValue *o)
{
    CTState *cts = ctype_ctsG(J2G(J));
    if (!tref_iscdata(tr))
    {
...
TRef
lj_crecord_topuint8(jit_State *J, TRef tr)
{
