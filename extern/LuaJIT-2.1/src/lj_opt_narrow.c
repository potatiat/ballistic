...
static BPropEntry *
narrow_bpc_get(jit_State *J, IRRef1 key, IRRef mode)
{
    ptrdiff_t i;
...
static void
narrow_bpc_set(jit_State *J, IRRef1 key, IRRef1 val, IRRef mode)
{
...
static void
narrow_stripov_backprop(NarrowConv *nc, IRRef ref, int depth)
{
    jit_State *J  = nc->J;
    IRIns     *ir = IR(ref);
...
static int
narrow_conv_backprop(NarrowConv *nc, IRRef ref, int depth)
{
    jit_State *J  = nc->J;
    IRIns     *ir = IR(ref);
    IRRef      cref;
...
static IRRef
narrow_conv_emit(jit_State *J, NarrowConv *nc)
{
    ...
    { /* Simple stack machine to process the ins. list. */
        ...
        else if (op == NARROW_INT)
        {
            ...
            *sp++ = nc->t == IRT_I64 ? lj_ir_kint64(J, (int64_t)(int32_t)*next++)
                                     : lj_ir_kint(J, *next++);
        }
        else
...
TRef LJ_FASTCALL
lj_opt_narrow_convert(jit_State *J)
{
...
static TRef
narrow_stripov(jit_State *J, TRef tr, int lastop, IRRef mode)
{
    ...
    if (op >= IR_ADDOV && op <= lastop)
    {
        ...
        else
        {
            ...
            tr  = emitir(
                IRT(op - IR_ADDOV + IR_ADD, ((mode & IRCONV_DSTMASK) >> IRCONV_DSH)), op1, op2);
...
/* Narrow array index. */
TRef LJ_FASTCALL
lj_opt_narrow_index(jit_State *J, TRef tr)
{
    IRIns *ir;
...
TRef LJ_FASTCALL
lj_opt_narrow_toint(jit_State *J, TRef tr)
{
    if (tref_isstr(tr))
    {
...
TRef LJ_FASTCALL
lj_opt_narrow_tobit(jit_State *J, TRef tr)
{
    if (tref_isstr(tr))
    {
        tr = emitir(IRTG(IR_STRTO, IRT_NUM), tr, 0);
...
TRef LJ_FASTCALL
lj_opt_narrow_cindex(jit_State *J, TRef tr)
{
    lj_assertJ(tref_isnumber(tr), "expected number type");
    if (tref_isnum(tr))
    {
...
static TRef
conv_str_tonum(jit_State *J, TRef tr, TValue *o)
{
    if (tref_isstr(tr))
    {
        tr = emitir(IRTG(IR_STRTO, IRT_NUM), tr, 0);
...
TRef
lj_opt_narrow_arith(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc, IROp op)
{
    rb = conv_str_tonum(J, rb, vb);
    rc = conv_str_tonum(J, rc, vc);
    /* Must not narrow MUL in non-DUALNUM variant, because it loses -0. */
    if ((op >= IR_ADD && op <= (LJ_DUALNUM ? IR_MUL : IR_SUB)) && tref_isinteger(rb)
        && tref_isinteger(rc)
        && lj_num2int_ok(lj_vm_foldarith(numberVnum(vb), numberVnum(vc), (int)op - (int)IR_ADD)))
    {
...
TRef
lj_opt_narrow_unm(jit_State *J, TRef rc, TValue *vc)
{
    rc = conv_str_tonum(J, rc, vc);
...
TRef
lj_opt_narrow_mod(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc)
{
    TRef tmp;
    rb = conv_str_tonum(J, rb, vb);
    rc = conv_str_tonum(J, rc, vc);
    if ((LJ_DUALNUM || (J->flags & JIT_F_OPT_NARROW)) && tref_isinteger(rb) && tref_isinteger(rc)
...
static int
narrow_forl(jit_State *J, cTValue *o)
{
    if (tvisint(o))
    {
        return 1;
...
IRType
lj_opt_narrow_forl(jit_State *J, cTValue *tv)
{
    lj_assertJ(tvisnumber(&tv[FORL_IDX]) && tvisnumber(&tv[FORL_STOP])
                   && tvisnumber(&tv[FORL_STEP]),
               "expected number types");
    /* Narrow only if the runtime values of start/stop/step are all integers. */
    if (narrow_forl(J, &tv[FORL_IDX]) && narrow_forl(J, &tv[FORL_STOP])
