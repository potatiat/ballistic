...
static TValue *
rec_stop_stitch_cp(lua_State *L, lua_CFunction dummy, void *ud)
{
...
static void LJ_FASTCALL
recff_nyi(jit_State *J, RecordFFData *rd)
{
...
/* Emit TMPREF. */
static TRef
recff_tmpref(jit_State *J, TRef tr, int mode)
{
...
static void LJ_FASTCALL
recff_assert(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_type(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_setmetatable(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
    TRef mt = J->base[1];
...
static void LJ_FASTCALL
recff_rawget(jit_State *J, RecordFFData *rd)
{
    RecordIndex ix;
    ix.tab = J->base[0];
    ix.key = J->base[1];
...
static void LJ_FASTCALL
recff_rawset(jit_State *J, RecordFFData *rd)
{
    RecordIndex ix;
    ix.tab = J->base[0];
    ix.key = J->base[1];
...
static void LJ_FASTCALL
recff_rawequal(jit_State *J, RecordFFData *rd)
{
    TRef tra = J->base[0];
    TRef trb = J->base[1];
    if (tra && trb)
...
#if LJ_52
static void LJ_FASTCALL
recff_rawlen(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
    if (tref_isstr(tr))
    {
...
int32_t
lj_ffrecord_select_mode(jit_State *J, TRef tr, TValue *tv)
{
...
static void LJ_FASTCALL
recff_select(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
    if (tr)
    {
...
static void LJ_FASTCALL
recff_tonumber(jit_State *J, RecordFFData *rd)
{
    TRef tr   = J->base[0];
    TRef base = J->base[1];
...
static TValue *
recff_metacall_cp(lua_State *L, lua_CFunction dummy, void *ud)
{
    jit_State *J = (jit_State *)ud;
...
static int
recff_metacall(jit_State *J, RecordFFData *rd, MMS mm)
{
    RecordIndex ix;
    ix.tab = J->base[0];
    copyTV(J->L, &ix.tabv, &rd->argv[0]);
...
static void LJ_FASTCALL
recff_tostring(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
    if (tref_isstr(tr))
    {
    ...
    else if (tr && !recff_metacall(J, rd, MM_tostring))
    {
        ...
        {
            J->base[0]
                = emitir(IRT(IR_TOSTR, IRT_STR), tr, tref_isnum(tr) ? IRTOSTR_NUM : IRTOSTR_INT);
        }
...
static void LJ_FASTCALL
recff_ipairs_aux(jit_State *J, RecordFFData *rd)
{
    RecordIndex ix;
    ix.tab = J->base[0];
...
static void LJ_FASTCALL
recff_xpairs(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
...
static void LJ_FASTCALL
recff_pcall(jit_State *J, RecordFFData *rd)
{
    if (J->maxslot >= 1)
    {
#if LJ_FR2
...
static TValue *
recff_xpcall_cp(lua_State *L, lua_CFunction dummy, void *ud)
{
    jit_State *J = (jit_State *)ud;
...
static void LJ_FASTCALL
recff_getfenv(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
...
static void LJ_FASTCALL
recff_next(jit_State *J, RecordFFData *rd)
{
#if LJ_BE
    ...
    */
    ...
    if (tref_istab(tab))
    {
        ...
        { /* Shortcut for start of traversal. */
...
static void LJ_FASTCALL
recff_math_abs(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_math_round(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
...
static void LJ_FASTCALL
recff_math_unary(jit_State *J, RecordFFData *rd)
{
...
/* Record math.log. */
static void LJ_FASTCALL
recff_math_log(jit_State *J, RecordFFData *rd)
{
    TRef tr = lj_ir_tonum(J, J->base[0]);
    if (J->base[1])
    {
...
/* Record math.atan2. */
static void LJ_FASTCALL
recff_math_atan2(jit_State *J, RecordFFData *rd)
{
    TRef tr    = lj_ir_tonum(J, J->base[0]);
...
/* Record math.ldexp. */
static void LJ_FASTCALL
recff_math_ldexp(jit_State *J, RecordFFData *rd)
{
    TRef tr = lj_ir_tonum(J, J->base[0]);
...
static void LJ_FASTCALL
recff_math_call(jit_State *J, RecordFFData *rd)
{
    TRef tr    = lj_ir_tonum(J, J->base[0]);
...
static void LJ_FASTCALL
recff_math_pow(jit_State *J, RecordFFData *rd)
{
    J->base[0] = lj_opt_narrow_arith(J, J->base[0], J->base[1], &rd->argv[0], &rd->argv[1], IR_POW);
    UNUSED(rd);
}

static void LJ_FASTCALL
recff_math_minmax(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_math_random(jit_State *J, RecordFFData *rd)
{
...
/* Record bit.tobit. */
static void LJ_FASTCALL
recff_bit_tobit(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
#if LJ_HASFFI
    if (tref_iscdata(tr))
...
static void LJ_FASTCALL
recff_bit_unary(jit_State *J, RecordFFData *rd)
{
#if LJ_HASFFI
...
static void LJ_FASTCALL
recff_bit_nary(jit_State *J, RecordFFData *rd)
{
#if LJ_HASFFI
...
/* Record bit shifts. */
static void LJ_FASTCALL
recff_bit_shift(jit_State *J, RecordFFData *rd)
{
#if LJ_HASFFI
...
static void LJ_FASTCALL
recff_bit_tohex(jit_State *J, RecordFFData *rd)
{
#if LJ_HASFFI
...
static TRef
recff_string_start(jit_State *J, GCstr *s, int32_t *st, TRef tr, TRef trlen, TRef tr0)
{
    int32_t start = *st;
    if (start < 0)
    {
...
static void LJ_FASTCALL
recff_string_range(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_string_char(jit_State *J, RecordFFData *rd)
{
    ...
    if (i > 1)
    { /* Concatenate the strings, if there's more than one. */
...
static void LJ_FASTCALL
recff_string_rep(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_string_op(jit_State *J, RecordFFData *rd)
{
    TRef str   = lj_ir_tostr(J, J->base[0]);
    TRef hdr   = recff_bufhdr(J);
...
static void LJ_FASTCALL
recff_string_find(jit_State *J, RecordFFData *rd)
{
    TRef    trstr = lj_ir_tostr(J, J->base[0]);
    ...
    if ((J->base[2] && tref_istruecond(J->base[3]))
        || (emitir(IRTG(IR_EQ, IRT_STR), trpat, lj_ir_kstr(J, pat)), !lj_str_haspattern(pat)))
    { /* Search for fixed string. */
        ...
        if (lj_str_find(
                strdata(str) + (MSize)start, strdata(pat), str->len - (MSize)start, pat->len))
        {
            TRef pos;
...
static void
recff_format(jit_State *J, RecordFFData *rd, TRef hdr, int sbufx)
{
    ptrdiff_t   arg = sbufx;
    ...
    { /* Parse format. */
        ...
        switch (STRFMT_TYPE(sf))
        {
            case STRFMT_LIT:
                tr = emitir(
                    IRTG(IR_BUFPUT, IRT_PGC), tr, lj_ir_kstr(J, lj_str_new(J->L, fs.str, fs.len)));
                break;
            case STRFMT_INT:
            ...
            handle_int:
                ...
                { /* Shortcut for plain %d. */
                    tr = emitir(IRTG(IR_BUFPUT, IRT_PGC),
                                tr,
...
#if LJ_HASFFI
                    tra = emitir(
                        IRT(IR_CONV, IRT_U64), tra, (IRT_INT | (IRT_U64 << 5) | IRCONV_SEXT));
                    tr = lj_ir_call(J, IRCALL_lj_strfmt_putfxint, tr, trsf, tra);
                    lj_needsplit(J);
#else
                    recff_nyiu(J, rd); /* Don't bother working around this NYI. */
...
static void LJ_FASTCALL
recff_string_format(jit_State *J, RecordFFData *rd)
{
...
static LJ_AINLINE TRef
recff_sbufx_get_L(jit_State *J, TRef ud)
{
    return emitir(IRT(IR_FLOAD, IRT_PGC), ud, IRFL_SBUF_L);
}

static LJ_AINLINE void
recff_sbufx_set_L(jit_State *J, TRef ud, TRef val)
{
...
static LJ_AINLINE TRef
recff_sbufx_get_ptr(jit_State *J, TRef ud, IRFieldID fl)
{
    return emitir(IRT(IR_FLOAD, IRT_PTR), ud, fl);
}

static LJ_AINLINE void
recff_sbufx_set_ptr(jit_State *J, TRef ud, IRFieldID fl, TRef val)
{
...
static LJ_AINLINE TRef
recff_sbufx_len(jit_State *J, TRef trr, TRef trw)
{
...
static TRef
recff_sbufx_check(jit_State *J, RecordFFData *rd, ptrdiff_t arg)
{
    TRef trtype, ud = J->base[arg];
    if (!tvisbuf(&rd->argv[arg]))
    {
        lj_trace_err(J, LJ_TRERR_BADTYPE);
    }
...
static TRef
recff_sbufx_write(jit_State *J, TRef ud)
{
...
static TRef
recff_sbufx_checkint(jit_State *J, RecordFFData *rd, ptrdiff_t arg)
{
    TRef tr    = J->base[arg];
...
static void LJ_FASTCALL
recff_buffer_method_reset(jit_State *J, RecordFFData *rd)
{
    ...
    if (iscow)
    {
        ...
        emitir(IRT(IR_FSTORE, IRT_PGC), emitir(IRT(IR_FREF, IRT_PGC), ud, IRFL_SBUF_REF), zeropgc);
...
static void LJ_FASTCALL
recff_buffer_method_skip(jit_State *J, RecordFFData *rd)
{
    TRef ud  = recff_sbufx_check(J, rd, 0);
...
static void LJ_FASTCALL
recff_buffer_method_set(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
...
static void LJ_FASTCALL
recff_buffer_method_put(jit_State *J, RecordFFData *rd)
{
    TRef      ud    = recff_sbufx_check(J, rd, 0);
    TRef      trbuf = recff_sbufx_write(J, ud);
    TRef      tr;
...
static void LJ_FASTCALL
recff_buffer_method_putf(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trbuf = recff_sbufx_write(J, ud);
...
static void LJ_FASTCALL
recff_buffer_method_get(jit_State *J, RecordFFData *rd)
{
    TRef      ud  = recff_sbufx_check(J, rd, 0);
    TRef      trr = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_R);
    TRef      trw = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_W);
...
static void LJ_FASTCALL
recff_buffer_method___tostring(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trr   = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_R);
    TRef trw   = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_W);
...
static void LJ_FASTCALL
recff_buffer_method___len(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trr   = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_R);
    TRef trw   = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_W);
...
#if LJ_HASFFI
static void LJ_FASTCALL
recff_buffer_method_putcdata(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trbuf = recff_sbufx_write(J, ud);
...
static void LJ_FASTCALL
recff_buffer_method_reserve(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trbuf = recff_sbufx_write(J, ud);
...
static void LJ_FASTCALL
recff_buffer_method_commit(jit_State *J, RecordFFData *rd)
{
    TRef ud   = recff_sbufx_check(J, rd, 0);
...
static void LJ_FASTCALL
recff_buffer_method_ref(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trr   = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_R);
    TRef trw   = recff_sbufx_get_ptr(J, ud, IRFL_SBUF_W);
...
static void LJ_FASTCALL
recff_buffer_method_encode(jit_State *J, RecordFFData *rd)
{
    TRef ud    = recff_sbufx_check(J, rd, 0);
    TRef trbuf = recff_sbufx_write(J, ud);
    TRef tmp   = recff_tmpref(J, J->base[1], IRTMPREF_IN1);
...
static void LJ_FASTCALL
recff_buffer_method_decode(jit_State *J, RecordFFData *rd)
{
    TRef   ud    = recff_sbufx_check(J, rd, 0);
    TRef   trbuf = recff_sbufx_write(J, ud);
...
static void LJ_FASTCALL
recff_buffer_encode(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_buffer_decode(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_table_insert(jit_State *J, RecordFFData *rd)
{
    RecordIndex ix;
    ix.tab   = J->base[0];
    ix.val   = J->base[1];
...
static void LJ_FASTCALL
recff_table_concat(jit_State *J, RecordFFData *rd)
{
    TRef tab = J->base[0];
    if (tref_istab(tab))
    {
        TRef sep = !tref_isnil(J->base[1]) ? lj_ir_tostr(J, J->base[1]) : lj_ir_knull(J, IRT_STR);
        TRef tri = (J->base[1] && !tref_isnil(J->base[2])) ? lj_opt_narrow_toint(J, J->base[2])
                                                           : lj_ir_kint(J, 1);
...
static void LJ_FASTCALL
recff_table_new(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_table_clear(jit_State *J, RecordFFData *rd)
{
    TRef tr = J->base[0];
    if (tref_istab(tr))
    {
...
*/
static TRef
recff_io_fp(jit_State *J, TRef *udp, int32_t id)
{
    TRef tr, ud, fp;
    if (id)
    { /* io.func() */
...
static void LJ_FASTCALL
recff_io_write(jit_State *J, RecordFFData *rd)
{
...
static void LJ_FASTCALL
recff_io_flush(jit_State *J, RecordFFData *rd)
{
    TRef ud, fp = recff_io_fp(J, &ud, rd->data);
...
static void LJ_FASTCALL
recff_debug_getmetatable(jit_State *J, RecordFFData *rd)
{
    GCtab *mt;
    TRef   mtref;
    TRef   tr = J->base[0];
