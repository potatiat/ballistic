...
static IRIns *
sink_checkalloc(jit_State *J, IRIns *irs)
{
    IRIns *ir = IR(irs->op1);
    if (!irref_isk(ir->op2))
    {
...
static int
sink_phidep(jit_State *J, IRRef ref, int *workp)
{
    IRIns *ir = IR(ref);
    if (!*workp)
    {
    ...
    if (ir->op1 >= REF_FIRST && sink_phidep(J, ir->op1, workp))
    {
        return 1;
    }
    if (ir->op2 >= REF_FIRST && sink_phidep(J, ir->op2, workp))
    {
        return 1;
    }
    return 0;
...
static int
sink_checkphi(jit_State *J, IRIns *ira, IRRef ref)
{
    if (ref >= REF_FIRST)
    {
        IRIns *ir = IR(ref);
        if (irt_isphi(ir->t)
            || (ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT && irt_isphi(IR(ir->op1)->t)))
        {
            ira->prev++;
...
static void
sink_mark_snap(jit_State *J, SnapShot *snap)
{
...
*/
void
lj_opt_sink(jit_State *J)
{
    const uint32_t need
        = (JIT_F_OPT_SINK | JIT_F_OPT_FWD | JIT_F_OPT_DCE | JIT_F_OPT_CSE | JIT_F_OPT_FOLD);
