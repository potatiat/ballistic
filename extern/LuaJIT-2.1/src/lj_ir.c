...
void LJ_FASTCALL
lj_ir_growtop(jit_State *J)
{
    IRIns *baseir = J->irbuf + J->irbotlim;
    MSize  szins  = J->irtoplim - J->irbotlim;
    if (szins)
    {
        baseir = (IRIns *)lj_mem_realloc(
            J->L, baseir, szins * sizeof(IRIns), 2 * szins * sizeof(IRIns));
...
TRef
lj_ir_call(jit_State *J, IRCallID id, ...)
{
...
TRef
lj_ir_ggfload(jit_State *J, IRType t, uintptr_t ofs)
{
...
*/
static LJ_AINLINE IRRef
ir_nextk(jit_State *J)
{
    IRRef ref = J->cur.nk;
...
TRef LJ_FASTCALL
lj_ir_kint(jit_State *J, int32_t k)
{
...
TRef
lj_ir_k64(jit_State *J, IROp op, uint64_t u64)
{
    IRIns *ir, *cir = J->cur.ir;
    IRRef  ref;
...
TRef
lj_ir_knum_u64(jit_State *J, uint64_t u64)
{
...
TRef
lj_ir_kint64(jit_State *J, uint64_t u64)
{
...
TRef
lj_ir_knumint(jit_State *J, lua_Number n)
{
    int64_t i64;
    int32_t k;
    TValue  tv;
...
TRef
lj_ir_kgc(jit_State *J, GCobj *o, IRType t)
{
    IRIns *ir, *cir = J->cur.ir;
    IRRef  ref;
...
TRef
lj_ir_kptr_(jit_State *J, IROp op, void *ptr)
{
    IRIns *ir, *cir = J->cur.ir;
    IRRef  ref;
#if LJ_64 && !LJ_GC64
...
TRef
lj_ir_knull(jit_State *J, IRType t)
{
    IRIns *ir, *cir = J->cur.ir;
    IRRef  ref;
...
/* Intern key slot. */
TRef
lj_ir_kslot(jit_State *J, TRef key, IRRef slot)
{
    IRIns *ir, *cir = J->cur.ir;
...
void
lj_ir_kvalue(lua_State *L, TValue *tv, const IRIns *ir)
{
    UNUSED(L);
...
#endif
        default:
            lj_assertL(0, "bad IR constant op %d", ir->o);
            break;
    }
}
...
TRef LJ_FASTCALL
lj_ir_tonumber(jit_State *J, TRef tr)
{
    if (!tref_isnumber(tr))
    {
        if (tref_isstr(tr))
...
TRef LJ_FASTCALL
lj_ir_tonum(jit_State *J, TRef tr)
{
    if (!tref_isnum(tr))
    {
...
TRef LJ_FASTCALL
lj_ir_tostr(jit_State *J, TRef tr)
{
    if (!tref_isstr(tr))
    {
        if (!tref_isnumber(tr))
...
int
lj_ir_numcmp(lua_Number a, lua_Number b, IROp op)
{
    switch (op)
    {
        case IR_EQ:
...
int
lj_ir_strcmp(GCstr *a, GCstr *b, IROp op)
{
...
void
lj_ir_rollback(jit_State *J, IRRef ref)
{
