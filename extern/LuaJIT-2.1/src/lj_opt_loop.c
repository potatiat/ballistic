...
static void
loop_emit_phi(jit_State *J, IRRef1 *subst, IRRef1 *phi, IRRef nphi, SnapNo onsnap)
{
    int   passx = 0;
    IRRef i, j, nslots;
    ...
    if (passx)
    {
        ...
        for (i = J->cur.nins - 1; i > invar; i--)
        {
            ...
            if (!irref_isk(ir->op1))
            {
                ...
                if (ir->op1 < invar && ir->o >= IR_CALLN && ir->o <= IR_CARG)
                { /* ORDER IR */
                    ir = IR(ir->op1);
                    while (ir->o == IR_CARG)
                    {
...
static void
loop_subst_snap(jit_State *J, SnapShot *osnap, SnapEntry *loopmap, IRRef1 *subst)
{
...
loop_unroll(LoopState *lps)
{
    ...
    *psentinel = SNAP(255, 0, 0); /* Replace PC with temporary sentinel. */
    ...
    for (ins = REF_FIRST; ins < invar; ins++)
    {
        ...
        if (irm_kind(lj_ir_mode[ir->o]) == IRM_N && op1 == ir->op1 && op2 == ir->op2)
            ...
            if (ref != ins)
            {
                ...
                { /* Loop-carried dependency? */
                    ...
                    if (!irt_sametype(t, irr->t))
                    {
                        ...
                        {
                            ref = tref_ref(
                                emitir(IRTGI(IR_CONV), ref, IRCONV_INT_NUM | IRCONV_CHECK));
                        }
                        else
                        {
                        ...
                        goto phiconv;
                         ...
                         && ((irr->o == IR_CONV && irr->op1 < invar)
                             || (irr->o == IR_ALEN && irr->op2 < invar && irr->op2 != REF_NIL)))
                {
...
static void
loop_undo(jit_State *J, IRRef ins, SnapNo nsnap, MSize nsnapmap)
{
    ptrdiff_t  i;
...
static TValue *
cploop_opt(lua_State *L, lua_CFunction dummy, void *ud)
{
    UNUSED(L);
    UNUSED(dummy);
