...
typedef long (*PNTAVM)(
    HANDLE handle, void **addr, ULONG_PTR zbits, size_t *size, ULONG alloctype, ULONG prot);
static PNTAVM ntavm;
...
static void *
mmap_plain(size_t size)
{
    DWORD olderr = GetLastError();
    void *ptr    = NULL;
    long  st     = ntavm(INVALID_HANDLE_VALUE,
                         &ptr,
                         NTAVM_ZEROBITS,
                         &size,
...
{
    DWORD olderr = GetLastError();
    void *ptr    = LJ_WIN_VALLOC(0, size, MEM_RESERVE | MEM_COMMIT | MEM_TOP_DOWN, PAGE_READWRITE);
    SetLastError(olderr);
    return ptr ? ptr : MFAIL;
}
...
static int
CALL_MUNMAP(void *ptr, size_t size)
{
    DWORD                    olderr = GetLastError();
...
static void *
mmap_probe(PRNGState *rs, size_t size)
{
...
#if LJ_ALLOC_MMAP_PROBE
static void *
mmap_map32(PRNGState *rs, size_t size)
#else
static void *
mmap_map32(size_t size)
#endif
{
#if LJ_ALLOC_MMAP_PROBE
...
#endif
    {
        int   olderr = errno;
        void *ptr
            = mmap((void *)LJ_ALLOC_MMAP32_START, size, MMAP_PROT, MAP_32BIT | MMAP_FLAGS, -1, 0);
        errno = olderr;
...
#if LJ_64 && !LJ_GC64                                                              \
    && ((defined(__FreeBSD__) && __FreeBSD__ < 10) || defined(__FreeBSD_kernel__)) \
...
static void *
CALL_MREMAP_(void *ptr, size_t osz, size_t nsz, int flags)
{
    int olderr = errno;
...
#define granularity_align(S) \
    (((S) + (DEFAULT_GRANULARITY - SIZE_T_ONE)) & ~(DEFAULT_GRANULARITY - SIZE_T_ONE))
...
static void *
direct_alloc(mstate m, size_t nb)
{
...
static mchunkptr
direct_resize(mchunkptr oldp, size_t nb)
{
...
static void
add_segment(mstate m, char *tbase, size_t tsize)
{
...
static int
alloc_trim(mstate m, size_t pad)
{
    size_t released = 0;
    if (pad < MAX_REQUEST && is_initialized(m))
    {
        ...
        if (m->topsize > pad)
        {
            ...
            if (sp->size >= extra && !has_segment_link(m, sp))
...
void
lj_alloc_setprng(void *msp, PRNGState *rs)
{
...
static LJ_NOINLINE void *
lj_alloc_malloc(void *msp, size_t nsize)
{
    mstate ms = (mstate)msp;
    void  *mem;
    size_t nb;
...
static LJ_NOINLINE void *
lj_alloc_free(void *msp, void *ptr)
{
    if (ptr != 0)
    {
...
static LJ_NOINLINE void *
lj_alloc_realloc(void *msp, void *ptr, size_t nsize)
{
