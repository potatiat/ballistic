...
static int
carith_checkarg(lua_State *L, CTState *cts, CDArith *ca)
{
    TValue *o  = L->base;
    int     ok = 1;
    MSize   i;
...
/* Pointer arithmetic. */
static int
carith_ptr(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
    CType    *ctp = ca->ct[0];
    uint8_t  *pp  = ca->p[0];
    ptrdiff_t idx;
    ...
    if (ctype_isptr(ctp->info) || ctype_isrefarray(ctp->info))
    {
        ...
        if (mm == MM_sub)
        {
            idx = (ptrdiff_t)(~(uintptr_t)idx + 1u);
        }
    }
...
static int
carith_int64(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
    if (ctype_isnum(ca->ct[0]->info) && ca->ct[0]->size <= 8 && ctype_isnum(ca->ct[1]->info)
        && ca->ct[1]->size <= 8)
    {
...
static int
lj_carith_meta(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
    cTValue *tv = NULL;
    if (tviscdata(L->base))
    {
...
int
lj_carith_op(lua_State *L, MMS mm)
{
...
B64DEF(sar64)
{
    return (uint64_t)((int64_t)x >> (sh & 63));
}
B64DEF(rol64)
{
    return lj_rol(x, (sh & 63));
}
B64DEF(ror64)
{
    return lj_ror(x, (sh & 63));
}

#undef B64DEF

uint64_t
lj_carith_shift64(uint64_t x, int32_t sh, int op)
{
    switch (op)
    {
...
uint64_t
lj_carith_check64(lua_State *L, int narg, CTypeID *id)
{
...
int64_t
lj_carith_mul64(int64_t a, int64_t b)
{
    return a * b;
}
#endif
...
uint64_t
lj_carith_divu64(uint64_t a, uint64_t b)
{
    if (b == 0)
    {
...
int64_t
lj_carith_divi64(int64_t a, int64_t b)
{
...
uint64_t
lj_carith_modu64(uint64_t a, uint64_t b)
{
    if (b == 0)
    {
        return U64x(80000000, 00000000);
...
int64_t
lj_carith_modi64(int64_t a, int64_t b)
{
    if (b == 0)
    {
        return U64x(80000000, 00000000);
...
/* Unsigned 64 bit x^k. */
uint64_t
lj_carith_powu64(uint64_t x, uint64_t k)
{
    uint64_t y;
    if (k == 0)
    {
...
/* Signed 64 bit x^k. */
int64_t
lj_carith_powi64(int64_t x, int64_t k)
{
    if (k == 0)
    {
        return 1;
