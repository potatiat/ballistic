...
static const char *
clib_extname(lua_State *L, const char *name)
{
    if (!strchr(name, '/')
#if LJ_TARGET_CYGWIN
        && !strchr(name, '\\')
...
static const char *
clib_check_lds(lua_State *L, const char *buf)
{
    const char *p, *e;
...
static const char *
clib_resolve_lds(lua_State *L, const char *name)
{
...
static void *
clib_loadlib(lua_State *L, const char *name, int global)
{
...
static void *
clib_getsym(CLibrary *cl, const char *name)
{
...
LJ_NORET LJ_NOINLINE static void
clib_error(lua_State *L, const char *fmt, const char *name)
{
            ...
            { /* Resolve default library handles (once). */
                ...
                switch (i)
                {
                    ...
                    case CLIB_HANDLE_KERNEL32:
                        h = LJ_WIN_LOADLIBA("kernel32.dll");
                        break;
                    case CLIB_HANDLE_USER32:
                        h = LJ_WIN_LOADLIBA("user32.dll");
                        break;
                    case CLIB_HANDLE_GDI32:
                        h = LJ_WIN_LOADLIBA("gdi32.dll");
                        break;
                }
                if (!h)
                {
...
static CTSize
clib_func_argsize(CTState *cts, CType *ct)
{
    CTSize n = 0;
    while (ct->sib)
    {
...
static const char *
clib_extsym(CTState *cts, CType *ct, GCstr *name)
{
    if (ct->sib)
    {
...
TValue *
lj_clib_index(lua_State *L, CLibrary *cl, GCstr *name)
{
        ...
        else
        {
            ...
            if (!p && ctype_isfunc(ct->info))
            {
                ...
                if (cconv == CTCC_FASTCALL || cconv == CTCC_STDCALL)
                {
                    ...
                    const char *symd
                        = lj_strfmt_pushf(L, cconv == CTCC_FASTCALL ? "@%s@%d" : "_%s@%d", sym, sz);
                    L->top--;
...
/* Load a C library. */
void
lj_clib_load(lua_State *L, GCtab *mt, GCstr *name, int global)
{
...
void
lj_clib_default(lua_State *L, GCtab *mt)
{
    CLibrary *cl = clib_new(L, mt);
