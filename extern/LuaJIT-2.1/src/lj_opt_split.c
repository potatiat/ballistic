...
static IRRef
split_emit(jit_State *J, uint16_t ot, IRRef1 op1, IRRef1 op2)
{
...
static IRRef
split_num2int(jit_State *J, IRRef lo, IRRef hi, int check)
{
    IRRef tmp, res;
#if LJ_LE
...
static IRRef
split_call_l(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir, IRCallID id)
{
...
static IRRef
split_call_li(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir, IRCallID id)
{
...
static IRRef
split_call_ll(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir, IRCallID id)
{
    IRRef tmp, op1 = ir->op1, op2 = ir->op2;
    J->cur.nins--;
#if LJ_LE
    ...
    ir->prev = tmp = split_emit(J, IRTI(IR_CALLN), tmp, id);
    return split_emit(
        J, IRT(IR_HIOP, (LJ_SOFTFP && irt_isnum(ir->t)) ? IRT_SOFTFP : IRT_INT), tmp, tmp);
}

/* Get a pointer to the other 32 bit word (LE: hiword, BE: loword). */
static IRRef
split_ptr(jit_State *J, IRIns *oir, IRRef ref)
{
...
#if LJ_HASFFI
static IRRef
split_bitshift(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *nir, IRIns *ir)
{
    IROp  op   = ir->o;
    IRRef kref = nir->op2;
    if (irref_isk(kref))
...
static IRRef
split_bitop(jit_State *J, IRRef1 *hisubst, IRIns *nir, IRIns *ir)
{
    IROp  op = ir->o;
...
static void
split_subst_snap(jit_State *J, SnapShot *snap, IRIns *oir)
{
    ...
    for (ref = REF_FIRST; ref < nins; ref++)
    {
            ...
            switch (ir->o)
            {
                ...
                default:
                    lj_assertJ(ir->o <= IR_NE || ir->o == IR_MIN || ir->o == IR_MAX,
                               ...
                               ir->o);
                    hi = split_emit(
                        J, IRTG(IR_HIOP, IRT_SOFTFP), hisubst[ir->op1], hisubst[ir->op2]);
                    break;
            }
        }
        else
        ...
        { /* See above, too. */
            ...
            { /* Conversion from 64 bit int. */
                ...
                {
                    split_call_l(
                        J, hisubst, oir, ir, st == IRT_I64 ? IRCALL_fp64_l2f : IRCALL_fp64_ul2f);
...
static TValue *
cpsplit(lua_State *L, lua_CFunction dummy, void *ud)
{
