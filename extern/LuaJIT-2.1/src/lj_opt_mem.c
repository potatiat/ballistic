...
static AliasRet
aa_escape(jit_State *J, IRIns *ir, IRIns *stop)
{
    IRRef ref = (IRRef)(ir - J->cur.ir); /* The ref that might be stored. */
    for (ir++; ir < stop; ir++)
    {
        if (ir->op2 == ref
            && (ir->o == IR_ASTORE || ir->o == IR_HSTORE || ir->o == IR_USTORE
                || ir->o == IR_FSTORE))
        {
...
static AliasRet
aa_table(jit_State *J, IRRef ta, IRRef tb)
{
...
static int
fwd_aa_tab_clear(jit_State *J, IRRef lim, IRRef ta)
{
...
int LJ_FASTCALL
lj_opt_fwd_tptr(jit_State *J, IRRef lim)
{
    IRRef ta  = fins->op1;
...
static AliasRet
aa_ahref(jit_State *J, IRIns *refa, IRIns *refb)
{
    IRRef  ka = refa->op2;
    IRRef  kb = refb->op2;
    IRIns *keya, *keyb;
...
static TRef
fwd_ahload(jit_State *J, IRRef xref)
{
    IRIns *xr  = IR(xref);
...
int LJ_FASTCALL
lj_opt_fwd_href_nokey(jit_State *J)
{
...
TRef LJ_FASTCALL
lj_opt_dse_ahstore(jit_State *J)
{
        ...
        if (IR(ref)->op1 == tab)
        {
            ...
            while (sref > ref)
            {
                ...
                { /* ASTORE to the same table. */
                    ...
                    if (!irt_isnil(store->t) && idx->o == IR_ADD && idx->op1 == ref
...
aa_uref(IRIns *refa, IRIns *refb)
{
    ...
    { /* Different functions, check disambiguation hash values. */
        ...
        else if (refa->o != refb->o)
        {
            ...
            if (irt_type(refa->t) == IRT_IGC)
            {
                refa->t.irt += IRT_PGC - IRT_IGC;
            }
            else if (irt_type(refb->t) == IRT_IGC)
            {
                refb->t.irt += IRT_PGC - IRT_IGC;
            }
            return ALIAS_NO;
        }
        else
...
*/
static AliasRet
aa_fref(jit_State *J, IRIns *refa, IRIns *refb)
{
    if (refa->op2 != refb->op2)
    {
    ...
    { /* Search for redundant or conflicting stores. */
        ...
        switch (aa_fref(J, xr, IR(store->op1)))
        {
            ...
            case ALIAS_MAY:
                ...
                break; /* Otherwise continue searching. */
            case ALIAS_MUST:
                if (store->op2 == val && !(xr->op2 >= IRFL_SBUF_W && xr->op2 <= IRFL_SBUF_R))
                {
...
int LJ_FASTCALL
lj_opt_fwd_sbuf(jit_State *J, IRRef lim)
{
    IRRef ref;
...
static IRIns *
aa_findcnew(jit_State *J, IRIns *ir)
{
    while (ir->o == IR_ADD)
    {
        if (!irref_isk(ir->op1))
...
static AliasRet
aa_cnew(jit_State *J, IRIns *refa, IRIns *refb)
{
...
static AliasRet
aa_xref(jit_State *J, IRIns *refa, IRIns *xa, IRIns *xb)
{
...
static IRRef
reassoc_trycse(jit_State *J, IROp op, IRRef op1, IRRef op2)
{
...
static IRRef
reassoc_xref(jit_State *J, IRIns *ir)
{
    ptrdiff_t ofs = 0;
    ...
    { /* Add of base + index. */
        ...
        if (ir1 != ir
            && !(idxref = reassoc_trycse(J, ir1->o, idxref, ir1->o == IR_BSHL ? ir1->op2 : idxref)))
        {
            return 0;
        }
...
cselim:
    ...
    if (!(fins->op2 & IRXLOAD_READONLY) && J->chain[IR_LOOP] && xref == fins->op1
        && (xref = reassoc_xref(J, xr)) != 0)
    {
        ref = J->chain[IR_XSTORE];
...
*/
int
lj_opt_fwd_wasnonnil(jit_State *J, IROpT loadop, IRRef xref)
{
    /* First check stores. */
