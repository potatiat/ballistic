...
serialize_more(char *w, SBufExt *sbx, MSize sz)
{
...
static LJ_NOINLINE char *
serialize_wu124_(char *w, uint32_t v)
{
    if (v < 0x1fe0)
    {
        v -= 0xe0;
...
static LJ_AINLINE char *
serialize_wu124(char *w, uint32_t v)
{
    if (LJ_LIKELY(v < 0xe0))
    {
        *w++ = (char)v;
...
static LJ_NOINLINE char *
serialize_ru124_(char *r, char *w, uint32_t *pv)
{
    uint32_t v = *pv;
    if (v != 0xff)
    {
...
static LJ_AINLINE char *
serialize_ru124(char *r, char *w, uint32_t *pv)
{
    if (LJ_LIKELY(r < w))
    {
...
void LJ_FASTCALL
lj_serialize_dict_prep_str(lua_State *L, GCtab *dict)
{
    if (!dict->hmask)
...
void LJ_FASTCALL
lj_serialize_dict_prep_mt(lua_State *L, GCtab *dict)
{
    if (!dict->hmask)
    { /* No hash part means not prepared, yet. */
        MSize i, len = lj_tab_len(dict);
...
static char *
serialize_put(char *w, SBufExt *sbx, cTValue *o)
{
    ...
    else if (tvistab(o))
    {
        ...
        if (sbx->depth <= 0)
        {
            lj_err_caller(sbufL(sbx), LJ_ERR_BUFFER_DEPTH);
        }
        sbx->depth--;
        if (t->asize > 0)
        ...
        *w++ = (char)(SER_TAG_TAB + (nhash ? 1 : 0) + (narray ? one : 0));
        if (narray)
        {
            w = serialize_wu124(w, narray);
        }
        if (nhash)
        {
            w = serialize_wu124(w, nhash);
        }
        if (narray)
        { /* Write array entries. */
...
#if LJ_BE
            {
                uint64_t u = lj_bswap64(*(uint64_t *)sp);
                memcpy(w, &u, 8);
            }
#else
            memcpy(w, sp, 8);
        ...
        else if (ctype_iscomplex(s->info) && s->size == 16)
        {
            ...
            *w++ = SER_TAG_COMPLEX;
            ...
            { /* Only swap the doubles. The re/im order stays the same. */
                uint64_t u = lj_bswap64(((uint64_t *)sp)[0]);
                memcpy(w, &u, 8);
                u = lj_bswap64(((uint64_t *)sp)[1]);
                memcpy(w + 8, &u, 8);
            }
#else
            memcpy(w, sp, 16);
...
static char *
serialize_get(char *r, SBufExt *sbx, TValue *o)
{
    char    *w = sbx->w;
    uint32_t tp;
    r = serialize_ru124(r, w, &tp);
    if (LJ_UNLIKELY(!r))
    {
        goto eob;
    }
    if (LJ_LIKELY(tp >= SER_TAG_STR))
    {
        ...
        if (LJ_UNLIKELY(len > (uint32_t)(w - r)))
        {
            goto eob;
        }
    ...
    else if (tp == SER_TAG_NUM)
    {
        ...
        if (!tvisnum(o))
        {
            setnanV(o); /* Fix non-canonical NaNs. */
        }
    }
    ...
    else if (tp >= SER_TAG_TAB && tp <= SER_TAG_DICT_MT)
    {
        ...
        if (tp == SER_TAG_DICT_MT)
        {
            ...
            if (!(tp >= SER_TAG_TAB && tp < SER_TAG_DICT_MT))
            {
                goto badtag;
            }
        }
    ...
    else if (tp >= SER_TAG_INT64 && tp <= SER_TAG_COMPLEX)
    {
        ...
        GCcdata *cd;
        if (LJ_UNLIKELY(r + sz > w))
        {
            goto eob;
        }
        if (LJ_UNLIKELY(!ctype_ctsG(G(sbufL(sbx)))))
        {
            goto badtag;
        }
        cd = lj_cdata_new_(sbufL(sbx),
                           tp == SER_TAG_INT64    ? CTID_INT64
                           : tp == SER_TAG_UINT64 ? CTID_UINT64
                                                  : CTID_COMPLEX_DOUBLE,
                           sz);
        memcpy(cdataptr(cd), r, sz);
        r += sz;
#if LJ_BE
...
/* Encode to buffer. */
SBufExt *LJ_FASTCALL
lj_serialize_put(SBufExt *sbx, cTValue *o)
{
...
/* Decode from buffer. */
char *LJ_FASTCALL
lj_serialize_get(SBufExt *sbx, TValue *o)
{
    sbx->depth = LJ_SERIALIZE_DEPTH;
...
GCstr *LJ_FASTCALL
lj_serialize_encode(lua_State *L, cTValue *o)
{
    SBufExt sbx;
    char   *w;
...
void
lj_serialize_decode(lua_State *L, TValue *o, GCstr *str)
{
    SBufExt sbx;
    char   *r;
    memset(&sbx, 0, sizeof(SBufExt));
...
LJ_FUNC MSize LJ_FASTCALL
lj_serialize_peektype(SBufExt *sbx)
{
    uint32_t tp;
