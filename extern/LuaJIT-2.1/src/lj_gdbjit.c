...
static uint32_t
gdbjit_strz(GDBJITctx *ctx, const char *str)
{
    uint8_t *p   = ctx->p;
...
static void
gdbjit_catnum(GDBJITctx *ctx, uint32_t n)
{
    if (n >= 10)
    {
        uint32_t m = n / 10;
...
/* Add a SLEB128 value. */
static void
gdbjit_sleb128(GDBJITctx *ctx, int32_t v)
{
    uint8_t *p = ctx->p;
...
gdbjit_ehframe(GDBJITctx *ctx)
{
    ...
    /* Registers saved in CFRAME. */
        ...
        DUV(2);
        DB(DW_CFA_offset | DW_REG_DI);
        DUV(3);
        DB(DW_CFA_offset | DW_REG_SI);
        DUV(4);
        DB(DW_CFA_offset | DW_REG_BX);
        DUV(5);
#elif LJ_TARGET_X64
        DB(DW_CFA_offset | DW_REG_BP);
        DUV(2);
        DB(DW_CFA_offset | DW_REG_BX);
        DUV(3);
        DB(DW_CFA_offset | DW_REG_15);
        DUV(4);
        DB(DW_CFA_offset | DW_REG_14);
        DUV(5);
        /* Extra registers saved for JIT-compiled code. */
        DB(DW_CFA_offset | DW_REG_13);
        DUV(LJ_GC64 ? 10 : 9);
        DB(DW_CFA_offset | DW_REG_12);
        DUV(LJ_GC64 ? 11 : 10);
#elif LJ_TARGET_ARM
        {
            int i;
...
static void LJ_FASTCALL
gdbjit_debuginfo(GDBJITctx *ctx)
{
    uint8_t *p = ctx->p;
...
static void LJ_FASTCALL
gdbjit_debugabbrev(GDBJITctx *ctx)
{
    uint8_t *p = ctx->p;
...
static void LJ_FASTCALL
gdbjit_debugline(GDBJITctx *ctx)
{
    uint8_t *p = ctx->p;
...
static void
gdbjit_initsect(GDBJITctx *ctx, int sect, GDBJITinitf initf)
{
    ctx->startp             = ctx->p;
...
static void
gdbjit_newentry(lua_State *L, GDBJITctx *ctx)
{
...
void
lj_gdbjit_addtrace(jit_State *J, GCtrace *T)
{
    GDBJITctx    ctx;
...
void
lj_gdbjit_deltrace(jit_State *J, GCtrace *T)
{
