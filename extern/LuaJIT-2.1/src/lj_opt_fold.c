...
#define gcstep_barrier(J, ref)                                                                   \
    ((ref) < J->chain[IR_LOOP]                                                                   \
     && (J->chain[IR_SNEW] || J->chain[IR_XSNEW] || J->chain[IR_TNEW] || J->chain[IR_TDUP]       \
         || J->chain[IR_CNEW] || J->chain[IR_CNEWI] || J->chain[IR_BUFSTR] || J->chain[IR_TOSTR] \
         || J->chain[IR_CALLA]))
...
static int32_t
kfold_intop(int32_t k1, int32_t k2, IROp op)
{
    switch (op)
    {
        case IR_ADD:
...
/* Forward declaration. */
static uint64_t kfold_int64arith(jit_State *J, uint64_t k1, uint64_t k2, IROp op);
...
LJFOLDF(kfold_intcomp)
{
    ...
    switch ((IROp)fins->o)
    {
        ...
        case IR_UGT:
            return CONDFOLD((uint32_t)a > (uint32_t)b);
        default:
            lj_assertJ(0, "bad IR op %d", fins->o);
            return FAILFOLD;
    }
}
...
static uint64_t
kfold_int64arith(jit_State *J, uint64_t k1, uint64_t k2, IROp op)
{
    UNUSED(J);
    switch (op)
    {
...
{
    return INT64FOLD(kfold_int64arith(J, ir_k64(fleft)->u64, ir_k64(fright)->u64, (IROp)fins->o));
}
...
LJFOLDF(bufput_bufstr)
{
    if ((J->flags & JIT_F_OPT_FWD))
    {
        ...
        if (fleft->o == IR_BUFHDR && fleft->op2 == IRBUFHDR_RESET && fleft->prev == hdr
            && fleft->op1 == IR(hdr)->op1 && !(irt_isphi(fright->t) && IR(hdr)->prev)
        ...
        if (IR(hdr)->op2 == IRBUFHDR_RESET && !irt_isphi(fright->t))
        {
            ...
            if (ir->o == IR_CALLL && ir->op2 >= IRCALL_lj_buf_putstr_reverse
...
LJFOLDF(bufput_kgc)
{
    if (LJ_LIKELY(J->flags & JIT_F_OPT_FOLD) && fright->o == IR_KGC)
    {
        ...
        {
            if (fleft->o == IR_BUFPUT && irref_isk(fleft->op2) && !irt_isphi(fleft->t))
...
{
    lj_assertJ(fleft->o == IR_BUFHDR || fleft->o == IR_BUFPUT || fleft->o == IR_CALLL,
               "bad buffer constructor IR op %d",
               fleft->o);
    ...
    if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE))
    {
        ...
        while (ref)
        {
            ...
            {
                lj_assertJ(ira->o == IR_BUFHDR || ira->o == IR_BUFPUT || ira->o == IR_CALLL
                               || ira->o == IR_CARG,
                           "bad buffer constructor IR op %d",
                           ira->o);
...
LJFOLDF(kfold_add_kgc)
{
    ...
    if (irt_iscdata(fleft->t))
    {
        ...
        if (ctype_isnum(ct->info) || ctype_isenum(ct->info) || ctype_isptr(ct->info)
            || ctype_isfunc(ct->info) || ctype_iscomplex(ct->info) || ctype_isvector(ct->info))
        {
...
{
    return lj_ir_kstr(J,
                      fins->op2 == IRTOSTR_INT ? lj_strfmt_int(J->L, fleft->i)
...
LJFOLDF(xstore_conv)
{
    ...
    if (!irt_is64(fins->t)
        ...
        && ((fright->op2 & IRCONV_SRCMASK) == IRT_I64 || (fright->op2 & IRCONV_SRCMASK) == IRT_U64))
    {
        fins->op2 = fright->op1;
        return RETRYFOLD;
    }
...
LJFOLDF(simplify_intsub_k)
{
    ...
    fins->op2
        = (IRRef1)lj_ir_kint(J, (int32_t)(~(uint32_t)fright->i + 1u)); /* Overflow for -2^31 ok. */
    return RETRYFOLD;
}
...
static TRef
simplify_intmul_k(jit_State *J, int32_t k)
{
...
LJFOLDF(simplify_shift_andk)
{
    ...
    PHIBARRIER(fright);
    if ((fins->o < IR_BROL ? LJ_TARGET_MASKSHIFT : LJ_TARGET_MASKROT) && irk->o == IR_KINT)
    { /* i o (j & mask) ==> i o j */
        int32_t mask = irt_is64(fins->t) ? 63 : 31;
...
LJFOLDF(simplify_shiftk_andk)
{
    ...
    {
        uint64_t k  = kfold_int64arith(J, ir_k64(irk)->u64, fright->i, (IROp)fins->o);
        IROpT    ot = fleft->ot;
        fins->op1   = fleft->op1;
        fins->op1   = (IRRef1)lj_opt_fold(J);
...
LJFOLDF(simplify_andor_k64)
{
    ...
    if (irk->o == IR_KINT64)
    {
        uint64_t k = kfold_int64arith(J, ir_k64(irk)->u64, ir_k64(fright)->u64, (IROp)fins->o);
        /* (i | k1) & k2 ==> i & k2, if (k1 & k2) == 0. */
        /* (i & k1) | k2 ==> i | k2, if (k1 | k2) == -1. */
...
LJFOLDF(abc_invar)
{
    ...
    if (!irt_isint(fins->t) && fins->op1 < J->chain[IR_LOOP]
        && (irt_isu32(fins->t) || (!irref_isk(fins->op1) && !irt_isphi(IR(fins->op1)->t))))
    {
        return DROPFOLD;
    }
    return NEXTFOLD;
    ...
    if (fins->op1 == fins->op2
        && (!irt_isnum(fins->t)
            || (fleft->o == IR_CONV && /* Converted integers cannot be NaN. */
                (uint32_t)(fleft->op2 & IRCONV_SRCMASK) - (uint32_t)IRT_I8
                    <= (uint32_t)(IRT_U64 - IRT_U8))))
    {
...
static TRef
kfold_xload(jit_State *J, IRIns *ir, const void *p)
{
    int32_t k;
...
#define FOLD_SNEW_TYPE8   IRT_U8 /* Prefer unsigned loads. */
    ...
    if (len <= FOLD_SNEW_MAX_LEN)
    {
        ...
        if (len > 0)
        {
            ...
            {
                tmp = emitir(
                    IRTI(IR_BAND), tmp, lj_ir_kint(J, LJ_ENDIAN_SELECT(0x00ffffff, 0xffffff00)));
            }
            fins->op1 = (IRRef1)tmp;
            fins->op2 = (IRRef1)val;
...
static TRef
merge_uref(jit_State *J, IRRef ref, IRIns *ir)
{
...
lj_opt_fold(jit_State *J)
{
    ...
    {
        lj_assertJ(
            ((JIT_F_OPT_FOLD | JIT_F_OPT_FWD | JIT_F_OPT_CSE | JIT_F_OPT_DSE) | JIT_F_OPT_DEFAULT)
                == JIT_F_OPT_DEFAULT,
        ...
        if ((J->flags & (JIT_F_OPT_FOLD | JIT_F_OPT_DSE)) != (JIT_F_OPT_FOLD | JIT_F_OPT_DSE)
...
TRef LJ_FASTCALL
lj_opt_cselim(jit_State *J, IRRef lim)
{
    IRRef  ref  = J->chain[fins->o];
    IRRef2 op12 = (IRRef2)fins->op1 + ((IRRef2)fins->op2 << 16);
    while (ref > lim)
