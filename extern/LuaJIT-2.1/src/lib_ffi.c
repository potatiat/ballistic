...
static GCcdata *
ffi_checkcdata(lua_State *L, int narg)
{
...
static void *
ffi_checkptr(lua_State *L, int narg, CTypeID id)
{
...
static int32_t
ffi_checkint(lua_State *L, int narg)
{
    CTState *cts = ctype_cts(L);
    TValue  *o   = L->base + narg - 1;
    int32_t  i;
...
static int
ffi_index_meta(lua_State *L, CTState *cts, CType *ct, MMS mm)
{
...
LJLIB_CF(ffi_meta___tostring)
{
    ...
    else
    {
        ...
        else if (ct->size == 8 && ctype_isinteger(ct->info))
        {
            setstrV(L,
                    L->top - 1,
                    lj_ctype_repr_int64(L, *(uint64_t *)cdataptr(cd), (ct->info & CTF_UNSIGNED)));
            goto checkgc;
        }
...
static int
ffi_callback_set(lua_State *L, GCfunc *fn)
{
    GCcdata *cd  = ffi_checkcdata(L, 1);
    CTState *cts = ctype_cts(L);
