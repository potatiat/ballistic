...
#define sbufX(sb) (lj_assertG_(G(sbufL(sb)), sbufisext(sb), "not an SBufExt"), (SBufExt *)(sb))
#define setsbufflag(sb, flag) (setmrefu((sb)->L, (flag)))

#define tvisbuf(o) (LJ_HASBUFFER && tvisudata(o) && udataV(o)->udtype == UDTYPE_BUFFER)
...
lj_buf_init(lua_State *L, SBuf *sb)
{
    setsbufL(sb, L);
    sb->w = sb->e = sb->b = NULL;
}
...
static LJ_AINLINE void
lj_buf_reset(SBuf *sb)
{
    sb->w = sb->b;
}

static LJ_AINLINE SBuf *
lj_buf_tmp_(lua_State *L)
{
    SBuf *sb = &G(L)->tmpbuf;
    setsbufL(sb, L);
    lj_buf_reset(sb);
...
static LJ_AINLINE void
lj_buf_free(global_State *g, SBuf *sb)
{
...
static LJ_AINLINE void
lj_bufx_init(lua_State *L, SBufExt *sbx)
{
...
static LJ_AINLINE void
lj_bufx_set_borrow(lua_State *L, SBufExt *sbx, SBuf *sb)
{
...
static LJ_AINLINE void
lj_bufx_set_cow(lua_State *L, SBufExt *sbx, const char *p, MSize len)
{
...
static LJ_AINLINE void
lj_bufx_free(lua_State *L, SBufExt *sbx)
{
    if (!sbufiscoworborrow(sbx))
    {
        lj_mem_free(G(L), sbx->b, sbufsz(sbx));
    }
    setsbufXL(sbx, L, SBUF_FLAG_EXT);
    setgcrefnull(sbx->cowref);
...
static LJ_AINLINE char *
lj_buf_wmem(char *p, const void *q, MSize len)
{
    return (char *)memcpy(p, q, len) + len;
}

static LJ_AINLINE void
lj_buf_putb(SBuf *sb, int c)
{
...
static LJ_AINLINE GCstr *
lj_buf_str(lua_State *L, SBuf *sb)
{
