...
void
lj_trace_err(jit_State *J, TraceError e)
{
...
void
lj_trace_err_info(jit_State *J, TraceError e)
{
    setintV(J->L->top++, (int32_t)e);
    lj_err_throw(J->L, LUA_ERRRUN);
}
...
GCtrace *LJ_FASTCALL
lj_trace_alloc(lua_State *L, GCtrace *T)
{
...
static void
trace_save(jit_State *J, GCtrace *T)
{
    size_t sztr  = ((sizeof(GCtrace) + 7) & ~7);
...
void LJ_FASTCALL
lj_trace_free(global_State *g, GCtrace *T)
{
    jit_State *J = G2J(g);
    if (T->traceno)
    {
...
void
lj_trace_reenableproto(GCproto *pt)
{
...
static void
trace_unpatch(jit_State *J, GCtrace *T)
{
    ...
    switch (bc_op(*pc))
    {
        ...
        case BC_JITERL:
        case BC_JLOOP:
            lj_assertJ(op == BC_ITERL || op == BC_ITERN || op == BC_LOOP || bc_isret(op),
                       "bad original bytecode %d",
                       op);
            *pc = T->startins;
            break;
        case BC_JFUNCF:
...
/* Flush a root trace. */
static void
trace_flushroot(jit_State *J, GCtrace *T)
{
    GCproto *pt = &gcref(T->startpt)->pt;
...
void
lj_trace_flush(jit_State *J, TraceNo traceno)
{
...
void
lj_trace_flushproto(global_State *g, GCproto *pt)
{
    while (pt->trace != 0)
    {
...
lj_trace_flushall(lua_State *L)
{
    ...
    lj_vmevent_send(J2G(J), TRACE, setstrV(V, V->top++, lj_str_newlit(V, "flush")););
    return 0;
}

/* Initialize JIT compiler state. */
void
lj_trace_initstate(global_State *g)
{
    jit_State *J = G2J(g);
    TValue    *tv;
...
void
lj_trace_freestate(global_State *g)
{
    jit_State *J = G2J(g);
...
static void
blacklist_pc(GCproto *pt, BCIns *pc)
{
...
static void
penalty_pc(jit_State *J, GCproto *pt, BCIns *pc, TraceError e)
{
...
trace_start(jit_State *J)
{
    ...
    { /* No free trace? */
        lj_assertJ((J2G(J)->hookmask & HOOK_GC) == 0, "recorder called from GC hook");
    ...
    lj_vmevent_send_(
        J2G(J), TRACE, TValue savetv = J2G(J)->tmptv; TValue savetv2 = J2G(J)->tmptv2;
...
trace_stop(jit_State *J)
{
    ...
    switch (op)
    {
        ...
        case BC_JMP:
            ...
            /* Avoid compiling a side trace twice (stack resizing uses parent exit). */
            {
                ...
                if (J->cur.topslot > snap->topslot)
                {
                    snap->topslot = J->cur.topslot;
                }
            }
    ...
    lj_vmevent_send(J2G(J), TRACE, setstrV(V, V->top++, lj_str_newlit(V, "stop"));
                    setintV(V->top++, traceno);
                    setfuncV(V, V->top++, J->fn););
}
...
trace_abort(jit_State *J)
{
    ...
    if (traceno)
    {
        ...
        lj_vmevent_send(
            J2G(J), TRACE, cTValue *bot = tvref(L->stack) + LJ_FR2; cTValue * frame;
            const BCIns *pc;
            BCPos        pos = 0;
    ...
    {
        if (!J->mcarea)
        { /* Disable JIT compiler if first mcode alloc fails. */
            J->flags &= ~JIT_F_ON;
...
trace_pendpatch(jit_State *J, int force)
{
...
static TValue *
trace_state(lua_State *L, lua_CFunction dummy, void *ud)
{
    ...
    retry:
        switch (J->state)
        {
            ...
            case LJ_TRACE_END:
                trace_pendpatch(J, 1);
                J->loopref = 0;
                if ((J->flags & JIT_F_OPT_LOOP) && J->cur.link == J->cur.traceno
...
void
lj_trace_ins(jit_State *J, const BCIns *pc)
{
...
void LJ_FASTCALL
lj_trace_hot(jit_State *J, const BCIns *pc)
{
...
static void
trace_hotside(jit_State *J, const BCIns *pc)
{
    SnapShot *snap = &traceref(J, J->parent)->snap[J->exitno];
    if (!(J2G(J)->hookmask & (HOOK_GC | HOOK_VMEVENT)) && isluafunc(curr_func(J->L))
        && snap->count != SNAPCOUNT_DONE && ++snap->count >= J->param[JIT_P_hotexit])
    {
...
void LJ_FASTCALL
lj_trace_stitch(jit_State *J, const BCIns *pc)
{
    /* Only start a new trace if not recording or inside __gc call or vmevent. */
    if (J->state == LJ_TRACE_IDLE && !(J2G(J)->hookmask & (HOOK_GC | HOOK_VMEVENT)))
    {
...
static TValue *
trace_exit_cp(lua_State *L, lua_CFunction dummy, void *ud)
{
...
static void
trace_exit_regs(lua_State *V, ExitState *ex)
{
    int32_t i;
...
static TraceNo
trace_exit_find(jit_State *J, MCode *pc)
{
    TraceNo traceno;
...
int LJ_FASTCALL
lj_trace_exit(jit_State *J, void *exptr)
{
    ERRNO_SAVE
    lua_State   *L  = J->L;
    ...
    if (exitcode)
    {
        copyTV(L, L->top++, &exiterr); /* Anchor the error object. */
    }

    if (!(LJ_HASPROFILE && (G(L)->hookmask & HOOK_PROFILE)))
    {
        lj_vmevent_send(
            G(L), TEXIT, lj_state_checkstack(V, 4 + RID_NUM_GPR + RID_NUM_FPR + LUA_MINSTACK);
            setintV(V->top++, J->parent);
            setintV(V->top++, J->exitno);
...
uintptr_t LJ_FASTCALL
lj_trace_unwind(jit_State *J, uintptr_t addr, ExitNo *ep)
{
