...
void
lj_mcode_sync(void *start, void *end)
{
#ifdef LUAJIT_USE_VALGRIND
...
static void *
mcode_alloc_at(uintptr_t hint, size_t sz, DWORD prot)
{
    return LJ_WIN_VALLOC((void *)hint, sz, MEM_RESERVE | MEM_COMMIT | MEM_TOP_DOWN, prot);
}

static void
mcode_free(void *p, size_t sz)
{
    UNUSED(sz);
...
static void
mcode_setprot(jit_State *J, void *p, size_t sz, DWORD prot)
{
#if LUAJIT_SECURITY_MCODE != 0
    DWORD oprot;
    if (!LJ_WIN_VPROTECT(p, sz, prot, &oprot))
    {
        mcode_protfail(J);
    }
#else
...
static void *
mcode_alloc_at(uintptr_t hint, size_t sz, int prot)
{
    void *p = mmap(
        (void *)hint, sz, prot | MCPROT_CREATE, MAP_PRIVATE | MAP_ANONYMOUS | MCMAP_CREATE, -1, 0);
    if (p == MAP_FAILED)
    {
        return NULL;
    }
...
static void
mcode_setprot(jit_State *J, void *p, size_t sz, int prot)
{
#if LUAJIT_SECURITY_MCODE != 0
#if MCMAP_CREATE
    UNUSED(J);
...
*/
static void *
mcode_alloc_at_TEST(jit_State *J, uintptr_t hint, size_t sz, int prot)
{
...
static void
mcode_protect(jit_State *J, int prot)
{
    UNUSED(J);
    UNUSED(prot);
}
...
static void
mcode_setrange(jit_State *J, uintptr_t addr)
{
#if LJ_TARGET_MIPS
    ...
    */
    ...
    if (addr < J->mcmin || !J->mcmin)
    {
        J->mcmin = 0x10000u;
    }
    if (addr > J->mcmax)
    {
        J->mcmax = ~(uintptr_t)0xffffu;
    }
}

/* Check if an address is in range of the mcode allocation range. */
static LJ_AINLINE int
mcode_inrange(jit_State *J, uintptr_t addr, size_t sz)
{
...
mcode_alloc(jit_State *J, size_t sz)
{
    ...
    if (p)
    {
        mcode_setprot(J, p, sz, MCPROT_GEN);
    }
#else
...
MCode *
lj_mcode_reserve(jit_State *J, MCode **lim)
{
    if (!J->mcarea)
    {
        mcode_allocarea(J);
...
void
lj_mcode_commit(jit_State *J, MCode *top)
{
    J->mctop = top;
...
MCode *
lj_mcode_patch(jit_State *J, MCode *ptr, int finish)
{
    if (finish)
    {
        if (J->mcarea == ptr)
...
void
lj_mcode_limiterr(jit_State *J, size_t need)
{
