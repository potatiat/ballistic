...
cconv_err_conv(CTState *cts, CType *d, CType *s, CTInfo flags)
{
    ...
    if ((flags & CCF_FROMTV))
    {
        src = lj_obj_typename[1
                              + (ctype_isnum(s->info)     ? LUA_TNUMBER
...
LJ_NORET static void
cconv_err_convtv(CTState *cts, CType *d, TValue *o, CTInfo flags)
{
    const char *dst = strdata(lj_ctype_repr(cts->L, ctype_typeid(cts, d), NULL));
...
static CType *
cconv_childqual(CTState *cts, CType *ct, CTInfo *qual)
{
...
*/
int
lj_cconv_compatptr(CTState *cts, CType *d, CType *s, CTInfo flags)
{
...
*/
void
lj_cconv_ct_ct(CTState *cts, CType *d, CType *s, uint8_t *dp, uint8_t *sp, CTInfo flags)
{
    CTSize dsize = d->size, ssize = s->size;
    CTInfo dinfo = d->info, sinfo = s->info;
    void  *tmpptr;

    lj_assertCTS(!ctype_isenum(dinfo) && !ctype_isenum(sinfo), "unresolved enum");
    lj_assertCTS(!ctype_isattrib(dinfo) && !ctype_isattrib(sinfo), "unstripped attribute");
    ...
    lj_assertCTS(!ctype_isbool(dinfo) || dsize == 1 || dsize == 4, "bad size for bool type");
    lj_assertCTS(!ctype_isbool(sinfo) || ssize == 1 || ssize == 4, "bad size for bool type");
...
int
lj_cconv_tv_ct(CTState *cts, CType *s, CTypeID sid, TValue *o, uint8_t *sp)
{
    CTInfo sinfo = s->info;
...
int
lj_cconv_tv_bf(CTState *cts, CType *s, TValue *o, uint8_t *sp)
{
    CTInfo   info = s->info;
    CTSize   pos, bsz;
    uint32_t val;
...
static void
cconv_array_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t, CTInfo flags)
{
    int32_t i;
...
static void
cconv_substruct_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t, int32_t *ip, CTInfo flags)
{
    CTypeID id = d->sib;
    while (id)
    {
...
static void
cconv_struct_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t, CTInfo flags)
{
    int32_t i = 0;
...
void
lj_cconv_ct_tv(CTState *cts, CType *d, uint8_t *dp, TValue *o, CTInfo flags)
{
...
void
lj_cconv_bf_tv(CTState *cts, CType *d, uint8_t *dp, TValue *o)
{
    CTInfo   info = d->info;
    CTSize   pos, bsz;
    uint32_t val, mask;
...
static void
cconv_array_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp, TValue *o, MSize len)
{
    CType *dc = ctype_rawchild(cts, d); /* Array element type. */
...
static void
cconv_substruct_init(CTState *cts, CType *d, uint8_t *dp, TValue *o, MSize len, MSize *ip)
{
    CTypeID id = d->sib;
    while (id)
    {
...
static void
cconv_struct_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp, TValue *o, MSize len)
{
    MSize i = 0;
...
*/
int
lj_cconv_multi_init(CTState *cts, CType *d, TValue *o)
{
...
void
lj_cconv_ct_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp, TValue *o, MSize len)
{
    if (len == 0)
    {
        memset(dp, 0, sz);
