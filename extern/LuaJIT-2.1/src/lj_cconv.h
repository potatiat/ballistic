...
cconv_idx(CTInfo info)
{
    uint32_t idx = ((info >> 26) & 15u); /* Dispatch bits. */
    lj_assertX(ctype_type(info) <= CT_MAYCONVERT, "cannot convert ctype %08x", info);
#if LJ_64
...
LJ_FUNC void lj_cconv_ct_ct(
    CTState *cts, CType *d, CType *s, uint8_t *dp, uint8_t *sp, CTInfo flags);
