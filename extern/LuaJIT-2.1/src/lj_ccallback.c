...
#define CALLBACK_SLOT2OFS(slot)                                                   \
    (CALLBACK_MCODE_HEAD + CALLBACK_MCODE_GROUP * ((slot) / CALLBACK_MCODE_NSLOT) \
     + CALLBACK_MCODE_SLOTSZ * (slot))
...
CALLBACK_OFS2SLOT(MSize ofs)
{
    ...
    return (ofs % (128 + CALLBACK_MCODE_GROUP)) / CALLBACK_MCODE_SLOTSZ
           + group * CALLBACK_MCODE_NSLOT;
}

#define CALLBACK_MAX_SLOT                                                         \
    (((CALLBACK_MCODE_SIZE - CALLBACK_MCODE_HEAD) / (CALLBACK_MCODE_GROUP + 128)) \
     * CALLBACK_MCODE_NSLOT)
...
static void *
callback_slot2ptr(CTState *cts, MSize slot)
{
...
MSize
lj_ccallback_ptr2slot(CTState *cts, void *p)
{
...
#elif LJ_TARGET_X86ORX64
static void *
callback_mcode_init(global_State *g, uint8_t *page)
{
    uint8_t *p      = page;
    ...
    *(void **)p = target;
    ...
    for (slot = 0; slot < CALLBACK_MAX_SLOT; slot++)
    {
        ...
        *p++ = (uint8_t)slot;
        ...
        {
            *p++ = XI_JMPs;
            *p++ = (uint8_t)(CALLBACK_MCODE_SLOTSZ
                                 * (CALLBACK_MCODE_NSLOT - 1 - (slot & (CALLBACK_MCODE_NSLOT - 1)))
                             - 2);
        }
    }
    return p;
...
#elif LJ_TARGET_ARM
static void *
callback_mcode_init(global_State *g, uint32_t *page)
{
    uint32_t *p      = page;
    ...
    *p++ = ARMI_STR | ARMI_LS_P | ARMI_LS_W | ARMF_D(RID_R12) | ARMF_N(RID_SP)
           | (CFRAME_SIZE - 4 * 9);
...
#if defined(LUAJIT_ENABLE_OSX_HRT) && LUAJIT_SECURITY_MCODE != 0 && defined(MAP_JIT) \
...
static void
callback_mcode_new(CTState *cts)
{
    ...
    p = mmap(NULL,
             sz,
             PROT_READ | PROT_WRITE | CCPROT_CREATE,
             MAP_PRIVATE | MAP_ANONYMOUS | CCMAP_CREATE,
             -1,
             0);
    if (p == MAP_FAILED)
    {
    ...
    UNUSED(pe);
    lj_assertCTS((size_t)((char *)pe - (char *)p) <= sz, "miscalculated CALLBACK_MAX_SLOT");
...
void
lj_ccallback_mcode_free(CTState *cts)
{
    size_t sz = (size_t)CALLBACK_MCODE_SIZE;
    void  *p  = cts->cb.mcode;
    if (p == NULL)
...
static void
callback_conv_args(CTState *cts, lua_State *L)
{
    TValue   *o       = L->top;
...
callback_conv_result(CTState *cts, lua_State *L, TValue *o)
{
    ...
    if (!ctype_isvoid(ctr->info))
    {
        ...
        if (ctype_isinteger_or_bool(ctr->info) && ctr->size < 4)
        {
            ...
            {
                *(uint32_t *)dp
                    = ctr->size == 1 ? (uint32_t)*(uint8_t *)dp : (uint32_t)*(uint16_t *)dp;
            }
            else
            {
...
static MSize
callback_slot_new(CTState *cts, CType *ct)
{
...
static CType *
callback_checkfunc(CTState *cts, CType *ct)
{
    int narg = 0;
    ...
    if (ctype_isfunc(ct->info))
    {
        ...
        CTypeID fid = ct->sib;
        if (!(ctype_isvoid(ctr->info) || ctype_isenum(ctr->info) || ctype_isptr(ctr->info)
...
void *
lj_ccallback_new(CTState *cts, CType *ct, GCfunc *fn)
{
