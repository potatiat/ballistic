...
GCcdata *
lj_cdata_newref(CTState *cts, const void *p, CTypeID id)
{
...
GCcdata *
lj_cdata_newv(lua_State *L, CTypeID id, CTSize sz, CTSize align)
{
    global_State *g;
...
GCcdata *
lj_cdata_newx(CTState *cts, CTypeID id, CTSize sz, CTInfo info)
{
...
void LJ_FASTCALL
lj_cdata_free(global_State *g, GCcdata *cd)
{
    ...
    else if (LJ_LIKELY(!cdataisv(cd)))
    {
        ...
        lj_assertG(ctype_hassize(ct->info) || ctype_isfunc(ct->info) || ctype_isextern(ct->info),
...
void
lj_cdata_setfin(lua_State *L, GCcdata *cd, GCobj *obj, uint32_t it)
{
...
CType *
lj_cdata_index(CTState *cts, GCcdata *cd, cTValue *key, uint8_t **pp, CTInfo *qual)
{
...
collect_attrib:
    ...
    { /* Integer cdata key. */
        ...
        {
            lj_cconv_ct_ct(
                cts, ctype_get(cts, CTID_INT_PSZ), ctk, (uint8_t *)&idx, cdataptr(cdk), 0);
            goto integer_key;
        }
    }
    else if (tvisstr(key))
...
static void
cdata_getconst(CTState *cts, TValue *o, CType *ct)
{
...
int
lj_cdata_get(CTState *cts, CType *s, TValue *o, uint8_t *sp)
{
    CTypeID sid;
    ...
    lj_assertCTS(ctype_ispointer(s->info) || ctype_isfield(s->info), "pointer or field expected");
...
void
lj_cdata_set(CTState *cts, CType *d, uint8_t *dp, TValue *o, CTInfo qual)
{
    ...
    /* Get child type of pointer/array/field. */
    lj_assertCTS(ctype_ispointer(d->info) || ctype_isfield(d->info), "pointer or field expected");
    d = ctype_child(cts, d);
