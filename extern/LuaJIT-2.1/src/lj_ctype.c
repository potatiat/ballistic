...
CTypeID
lj_ctype_new(CTState *cts, CType **ctp)
{
    CTypeID id = cts->top;
    CType  *ct;
...
CTypeID
lj_ctype_intern(CTState *cts, CTInfo info, CTSize size)
{
...
static void
ctype_addtype(CTState *cts, CType *ct, CTypeID id)
{
...
void
lj_ctype_addname(CTState *cts, CType *ct, CTypeID id)
{
...
CTypeID
lj_ctype_getname(CTState *cts, CType **ctp, GCstr *name, uint32_t tmask)
{
...
CType *
lj_ctype_getfieldq(CTState *cts, CType *ct, GCstr *name, CTSize *ofs, CTInfo *qual)
{
    while (ct->sib)
    {
...
CType *
lj_ctype_rawref(CTState *cts, CTypeID id)
{
    CType *ct = ctype_get(cts, id);
...
CTSize
lj_ctype_size(CTState *cts, CTypeID id)
{
...
CTSize
lj_ctype_vlsize(CTState *cts, CType *ct, CTSize nelem)
{
    uint64_t xsz = 0;
...
CTInfo
lj_ctype_info(CTState *cts, CTypeID id, CTSize *szp)
{
    CTInfo qual = 0;
    CType *ct   = ctype_get(cts, id);
    for (;;)
...
CTInfo
lj_ctype_info_raw(CTState *cts, CTypeID id, CTSize *szp)
{
    CType *ct = ctype_get(cts, id);
...
cTValue *
lj_ctype_meta(CTState *cts, CTypeID id, MMS mm)
{
    CType   *ct = ctype_get(cts, id);
    cTValue *tv;
    while (ctype_isattrib(ct->info) || ctype_isref(ct->info))
...
/* Prepend string. */
static void
ctype_prepstr(CTRepr *ctr, const char *str, MSize len)
{
    char *p = ctr->pb;
    if (ctr->buf + len + 1 > p)
    {
...
/* Prepend number. */
static void
ctype_prepnum(CTRepr *ctr, uint32_t n)
{
    char *p = ctr->pb;
    if (ctr->buf + 10 + 1 > p)
    {
...
/* Append number. */
static void
ctype_appnum(CTRepr *ctr, uint32_t n)
{
    char  buf[10];
...
/* Prepend qualifiers. */
static void
ctype_prepqual(CTRepr *ctr, CTInfo info)
{
...
/* Prepend named type. */
static void
ctype_preptype(CTRepr *ctr, CType *ct, CTInfo qual, const char *t)
{
    if (gcref(ct->name))
    {
...
static void
ctype_repr(CTRepr *ctr, CTypeID id)
{
...
GCstr *
lj_ctype_repr(lua_State *L, CTypeID id, GCstr *name)
{
    global_State *g = G(L);
    CTRepr        ctr;
...
GCstr *
lj_ctype_repr_int64(lua_State *L, uint64_t n, int isunsigned)
{
    char  buf[1 + 20 + 3];
    char *p    = buf + sizeof(buf);
    int   sign = 0;
...
GCstr *
lj_ctype_repr_complex(lua_State *L, void *sp, CTSize size)
{
...
void
lj_ctype_freestate(global_State *g)
{
