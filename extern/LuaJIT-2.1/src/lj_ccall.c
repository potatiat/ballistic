...
#define CCALL_PUSH(arg) *(GPRArg *)((uint8_t *)cc->stack + nsp) = (GPRArg)(arg), nsp += CTSIZE_PTR
...
#define CCALL_HANDLE_STRUCTRET2                        \
    ...
    if ((cl & 4))                                      \
    { /* Combine float HFA from separate registers. */ \
        CTSize i = (cl >> 8) - 1;                      \
        do                                             \
        {                                              \
...
#define CCALL_HANDLE_STRUCTARG                       \
    ...
    { /* Pass struct in FPRs or on stack. */         \
        isfp = (cl & 4) ? 2 : 1;                     \
...
#define CCALL_HANDLE_REGARG                                                             \
    ...
    { /* Try to pass argument in FPRs. */                                               \
        int n2 = ctype_isvector(d->info) ? 1 : isfp == 1 ? n : (d->size >> (4 - isfp)); \
        if (nfpr + n2 <= CCALL_NARG_FPR)                                                \
        {                                                                               \
            dp = &cc->fpr[nfpr];                                                        \
            nfpr += n2;                                                                 \
...
static int
ccall_classify_struct(CTState *cts, CType *ct)
{
    CTSize sz = ct->size;
...
/* Classify a C type. */
static void
ccall_classify_ct(CTState *cts, CType *ct, int *rcl, CTSize ofs)
{
    ...
    else
    {
        ...
        lj_assertCTS(ctype_hassize(ct->info), "classify ctype %08x without size", ct->info);
        if ((ofs & (ct->size - 1)))
        {
...
static int
ccall_classify_struct(CTState *cts, CType *ct, int *rcl, CTSize ofs)
{
    if (ct->size > 16)
    {
...
static int
ccall_struct_reg(CCallState *cc, CTState *cts, GPRArg *dp, int *rcl)
{
...
static int
ccall_struct_arg(CCallState *cc, CTState *cts, CType *d, int *rcl, TValue *o, int narg)
{
    GPRArg dp[2];
    dp[0] = dp[1] = 0;
...
static void
ccall_struct_ret(CCallState *cc, int *rcl, uint8_t *dp, CTSize sz)
{
    GPRArg   sp[2];
    MSize    ngpr = 0, nfpr = 0;
    uint32_t i;
...
static unsigned int
ccall_classify_struct(CTState *cts, CType *ct, CType *ctf)
{
    CTSize       sz = ct->size;
...
static CTInfo
ccall_struct_align_arm64(CTState *cts, CType *ct)
{
    CTSize sz;
    if (ct->sib)
    {
        while (ct->sib)
        {
            ...
            if (ctype_isfield(ct->info))
            {
                if ((ct->info & CTF_ALIGN) > CTALIGN_PTR)
                {
                    return CTALIGN(4);
                }
            }
...
ccall_classify_struct(CTState *cts, CType *ct)
{
    ...
    while (ct->sib && n <= 4)
    {
        ...
        {
            sct = ctype_rawchild(cts, ct);
            if (ctype_isarray(sct->info) && !sct->size)
            {
                goto noth;
            }
...
static void
ccall_copy_struct(CCallState *cc, CType *ctr, void *dp, void *sp, int ft)
{
...
*/
CTypeID
lj_ccall_ctid_vararg(CTState *cts, cTValue *o)
{
    if (tvisnumber(o))
    {
        return CTID_DOUBLE;
    ...
    else if (tviscdata(o))
    {
        ...
        if (ctype_isrefarray(s->info))
        {
            return lj_ctype_intern(
                cts, CTINFO(CT_PTR, CTALIGN_PTR | ctype_cid(s->info)), CTSIZE_PTR);
        }
...
*/
static int
ccall_set_args(lua_State *L, CTState *cts, CType *ct, CCallState *cc)
{
    int     gcsteps = 0;
    TValue *o, *top = L->top;
    CTypeID fid;
    ...
    for (o = L->base + 1, narg = 1; o < top; o++, narg++)
    {
        ...
        { /* Assumes LJ_LE. */
            ...
            {
                *(uint32_t *)dp
                    = d->size == 1 ? (uint32_t)*(uint8_t *)dp : (uint32_t)*(uint16_t *)dp;
            }
            else
            {
...
static int
ccall_get_results(lua_State *L, CTState *cts, CType *ct, CCallState *cc, int *ret)
{
...
/* Call C function. */
int
lj_ccall_func(lua_State *L, GCcdata *cd)
{
