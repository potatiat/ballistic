...
void
lj_snap_grow_buf_(jit_State *J, MSize need)
{
...
void
lj_snap_grow_map_(jit_State *J, MSize need)
{
    ...
    J->snapmapbuf = (SnapEntry *)lj_mem_realloc(
        J->L, J->snapmapbuf, J->sizesnapmap * sizeof(SnapEntry), need * sizeof(SnapEntry));
...
static MSize
snapshot_slots(jit_State *J, SnapEntry *map, BCReg nslots)
{
    ...
    for (s = 0; s < nslots; s++)
    {
...
#if LJ_FR2
        if (s == 1)
        { /* Ignore slot 1 in LJ_FR2 mode, except if tailcalled. */
            if ((tr & TREF_FRAME))
            {
        ...
        if (ref)
        {
            ...
            if ((LJ_FR2 || !(sn & (SNAP_CONT | SNAP_FRAME))) && ir->o == IR_SLOAD && ir->op1 == s
                && ref > retf)
            {
                /*
...
static MSize
snapshot_framelinks(jit_State *J, SnapEntry *map, uint8_t *topslot)
{
...
#endif
    lj_assertJ(!J->pt || (J->pc >= proto_bc(J->pt) && J->pc < proto_bc(J->pt) + J->pt->sizebc),
               "bad snapshot PC");
    while (frame > lim)
...
static void
snapshot_stack(jit_State *J, SnapShot *snap, MSize nsnapmap)
{
...
static BCReg
snap_usedef(jit_State *J, uint8_t *udf, const BCIns *pc, BCReg maxslot)
{
    BCReg  s;
    GCobj *o;
...
static void
snap_useuv(GCproto *pt, uint8_t *udf)
{
...
*/
static BloomFilter
snap_renamefilter(GCtrace *T, SnapNo lim)
{
    BloomFilter rfilt = 0;
    IRIns      *ir;
...
static RegSP
snap_renameref(GCtrace *T, SnapNo lim, IRRef ref, RegSP rs)
{
    IRIns *ir;
    for (ir = &T->ir[T->nins - 1]; ir->o == IR_RENAME; ir--)
    {
...
IRIns *
lj_snap_regspmap(jit_State *J, GCtrace *T, SnapNo snapno, IRIns *ir)
{
...
static TRef
snap_replay_const(jit_State *J, IRIns *ir)
{
    ...
    switch ((IROp)ir->o)
    {
        ...
        case IR_KNULL:
            return lj_ir_knull(J, irt_type(ir->t));
        default:
            lj_assertJ(0, "bad IR constant op %d", ir->o);
            return TREF_NIL;
    }
}
...
static TRef
snap_dedup(jit_State *J, SnapEntry *map, MSize nmax, IRRef ref)
{
    MSize j;
    for (j = 0; j < nmax; j++)
    {
...
static TRef
snap_pref(jit_State *J, GCtrace *T, SnapEntry *map, MSize nmax, BloomFilter seen, IRRef ref)
{
    IRIns *ir = &T->ir[ref];
    TRef   tr;
    if (irref_isk(ref))
...
static int
snap_sunk_store2(GCtrace *T, IRIns *ira, IRIns *irs)
{
...
snap_sunk_store(GCtrace *T, IRIns *ira, IRIns *irs)
{
    if (irs->s != 255)
    {
...
void
lj_snap_replay(jit_State *J, GCtrace *T)
{
    ...
    for (n = 0; n < nent; n++)
    {
        ...
        else
        {
            ...
            if (LJ_SOFTFP32 && (sn & SNAP_SOFTFPNUM))
            {
                t = IRT_NUM;
            }
            if (ir->o == IR_SLOAD)
    ...
    if (pass23)
    {
        ...
        for (n = 0; n < nent; n++)
        {
            ...
            if (regsp_reg(ir->r) == RID_SUNK)
            {
                ...
                pass23 = 1;
                lj_assertJ(ir->o == IR_TNEW || ir->o == IR_TDUP || ir->o == IR_CNEW
                               || ir->o == IR_CNEWI,
                           "sunk parent IR %04d has bad op %d",
                           refp - REF_BIAS,
                           ir->o);
                m = lj_ir_mode[ir->o];
                if (irm_op1(m) == IRMref)
                ...
                else
                {
                    ...
                    for (irs = ir + 1; irs < irlast; irs++)
                    {
                        if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs))
                        {
                            ...
                            else if ((LJ_SOFTFP32 || (LJ_32 && LJ_HASFFI)) && irs + 1 < irlast
                                     && (irs + 1)->o == IR_HIOP)
                            {
            ...
            if (regsp_reg(ir->r) == RID_SUNK)
            {
                ...
                if (LJ_HASFFI && ir->o == IR_CNEWI)
                {
                    if (LJ_32 && refp + 1 < T->nins && (ir + 1)->o == IR_HIOP)
                    {
                        ...
                        op2 = emitir_raw(IRT(IR_HIOP, IRT_I64),
                                         op2,
                                         snap_pref(J, T, map, nent, seen, (ir + 1)->op2));
                    }
                    ...
                    for (irs = ir + 1; irs < irlast; irs++)
                    {
                        if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs))
                        {
                            ...
                            if (irr->o != IR_FREF)
                            {
                                ...
                                {
                                    key = lj_ir_kslot(
                                        J, snap_replay_const(J, &T->ir[irk->op1]), irk->op2);
                                }
                                else
                                {
...
static void
snap_restoreval(
    jit_State *J, GCtrace *T, ExitState *ex, SnapNo snapno, BloomFilter rfilt, IRRef ref, TValue *o)
{
    IRIns  *ir = &T->ir[ref];
    IRType1 t  = ir->t;
    RegSP   rs = ir->prev;
    ...
    { /* Restore from register. */
        ...
        {
            lj_assertJ(ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT,
                       "restore from IR %04d has no reg",
                       ref - REF_BIAS);
                 ...
                 CTSize      sz)
{
    ...
    lj_assertJ(sz == 1 || sz == 2 || sz == 4 || sz == 8,
               "restore from IR %04d with bad size %d",
               ref - REF_BIAS,
               sz);
    if (sz == 4)
    {
...
static void
snap_unsink(
    jit_State *J, GCtrace *T, ExitState *ex, SnapNo snapno, BloomFilter rfilt, IRIns *ir, TValue *o)
{
    lj_assertJ(ir->o == IR_TNEW || ir->o == IR_TDUP || ir->o == IR_CNEW || ir->o == IR_CNEWI,
               "sunk allocation with bad op %d",
               ir->o);
#if LJ_HASFFI
    if (ir->o == IR_CNEW || ir->o == IR_CNEWI)
    {
        ...
        if (ir->o == IR_CNEWI)
        {
            ...
            {
                snap_restoredata(J, T, ex, snapno, rfilt, (ir + 1)->op2, LJ_LE ? p + 4 : p, 4);
                if (LJ_BE)
                {
                    p += 4;
                }
            ...
            for (irs = ir + 1; irs < irlast; irs++)
            {
                if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs))
                {
                    ...
                    lj_assertJ(T->ir[irs->op1].o == IR_ADD,
                               "sunk store with bad add op %d",
                               T->ir[irs->op1].o);
                    lj_assertJ(iro->o == IR_KINT || iro->o == IR_KINT64,
                               "sunk store with bad const offset op %d",
                               iro->o);
                    if (irt_is64(irs->t))
                    {
                        szs = 8;
                    ...
                    if (LJ_32 && irs + 1 < T->ir + T->nins && (irs + 1)->o == IR_HIOP)
                    {
                        ...
                        snap_restoredata(
                            J, T, ex, snapno, rfilt, (irs + 1)->op2, LJ_LE ? p + 4 : p, 4);
                        if (LJ_BE)
                        {
                            p += 4;
                        }
...
#endif
    {
        ...
        for (irs = ir + 1; irs < irlast; irs++)
        {
            if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs))
            {
                IRIns *irk = &T->ir[irs->op1];
                TValue tmp, *val;
                lj_assertJ(irs->o == IR_ASTORE || irs->o == IR_HSTORE || irs->o == IR_FSTORE,
                           "sunk store with bad op %d",
                           irs->o);
                if (irk->o == IR_FREF)
                {
                    switch (irk->op2)
...
const BCIns *
lj_snap_restore(jit_State *J, void *exptr)
{
