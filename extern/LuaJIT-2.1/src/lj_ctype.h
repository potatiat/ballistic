...
#define ctype_isxattrib(info, at) \
    (((info) & (CTMASK_NUM | CTATTRIB(CTMASK_ATTRIB))) == CTINFO(CT_ATTRIB, CTATTRIB(at)))
...
#define CTOKDEF(_)                                                                                \
    _(IDENT, "<identifier>")                                                                      \
    _(STRING, "<string>")                                                                         \
    _(INTEGER, "<integer>") _(EOF, "<eof>") _(OROR, "||") _(ANDAND, "&&") _(EQ, "==") _(NE, "!=") \
...
#define CDSDEF(_)                                                                               \
    _(VOID)                                                                                     \
    _(BOOL)                                                                                     \
    _(CHAR) _(INT) _(FP) _(LONG) _(LONGLONG) _(SHORT) _(COMPLEX) _(SIGNED) _(UNSIGNED) _(CONST) \
        _(VOLATILE) _(RESTRICT) _(INLINE) _(TYPEDEF) _(EXTERN) _(STATIC) _(AUTO) _(REGISTER)

/* C keywords. */
#define CKWDEF(_)                                                                           \
    CDSDEF(_)                                                                               \
    _(EXTENSION)                                                                            \
    _(ASM) _(ATTRIBUTE) _(DECLSPEC) _(CCDECL) _(PTRSZ) _(STRUCT) _(UNION) _(ENUM) _(SIZEOF) \
        _(ALIGNOF)
...
/* Get C type state. */
static LJ_AINLINE CTState *
ctype_cts(lua_State *L)
{
...
#define LJ_CTYPE_RESTORE(cts) \
    ((cts)->top = savects_.top, memcpy((cts)->hash, savects_.hash, sizeof(savects_.hash)))

/* Check C type ID for validity when assertions are enabled. */
static LJ_AINLINE CTypeID
ctype_check(CTState *cts, CTypeID id)
{
    UNUSED(cts);
...
static LJ_AINLINE CType *
ctype_get(CTState *cts, CTypeID id)
{
...
/* Get child C type. */
static LJ_AINLINE CType *
ctype_child(CTState *cts, CType *ct)
{
    lj_assertCTS(
...
static LJ_AINLINE CType *
ctype_raw(CTState *cts, CTypeID id)
{
...
static LJ_AINLINE CType *
ctype_rawchild(CTState *cts, CType *ct)
{
    do
    {
        ct = ctype_child(cts, ct);
...
ctype_setname(CType *ct, GCstr *s)
{
